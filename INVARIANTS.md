...
<!-- invariants:totals theorems=3011 files=95 examples=900 -->
...
## Optional early text-console geometry (`LeanOS/BootTextConsole.lean`)
...
## The one list of names the C code may use for kernel states, commands, and replies (`LeanOS/BoundaryVocabulary.lean`)
...
## Qotom J1900 CPU and control checkpoint (`LeanOS/J1900CpuControlPolicy.lean`)
...
## Checking the measured J1900 CPU capabilities (`LeanOS/J1900CpuProfile.lean`)
...
## Binding the J1900 CPU to its control readback (`LeanOS/J1900EntryControl.lean`)
...
## Exact Intel MSR readback (`LeanOS/J1900MsrReadback.lean`)
...
## Kernel-root publication and cached authority (`LeanOS/KernelRootPublication.lean`)
...
## Closed-root preparation and terminal return tail (`LeanOS/KernelRootReturn.lean`)
...
## Closing kernel mappings by physical frame (`LeanOS/KernelUserRoot.lean`)
...
## Bounded PCI header observations (`LeanOS/PCIHeaderObservation.lean`)
...
## Closed platform admission (`LeanOS/PlatformAdmission.lean`)
...
## Qotom blocking IPC profile admission (`LeanOS/QotomBlockingIPCIntegration.lean`)

This module admits only the fixed two-subject Qotom blocking-IPC profile after its earlier platform checkpoints, three distinct roots, exact root scans and entry gates, masked interrupt sources, two saved contexts, and generated-model bindings all agree. Its scalar results describe the one bounded trace and do not prove that hardware supplied the observations.

- `cpl3_authority_requires_admission` — If the profile reports CPL3 authority, every scalar platform, root, gate, interrupt, context-bank, and model-binding condition in the complete Qotom blocking-IPC admission predicate must have passed.

## Bounded Qotom bootstrap result queries (`LeanOS/QotomBootstrapABI.lean`)

This hosted interface exposes the combined topology and bootstrap candidate. Its results do not grant platform or runtime admission.

- `out_of_range_is_zero` — Every query beyond the five defined result words returns zero for arbitrary supplied inputs.

## Checking the captured Qotom processor inventory (`LeanOS/QotomBspTopology.lean`)
...
## Qotom copy-root publication checkpoint (`LeanOS/QotomCopyRootPublication.lean`)
...
## Qotom CPL3 entry/return checkpoint (`LeanOS/QotomEntryIntegration.lean`)
...
## Qotom scalar MADT candidate (`LeanOS/QotomMadtStream.lean`)
...
## Qotom scalar stream traversal model (`LeanOS/QotomMadtStreamRun.lean`)
...
## Scalar native PCI inventory fields (`LeanOS/QotomNativePCIFields.lean`)
...
## Native Qotom PCI inventory observations (`LeanOS/QotomNativePCIInventory.lean`)
...
## Complete native PCI snapshot binding (`LeanOS/QotomNativePCISnapshot.lean`)
...
## Qotom no-SMAP control checkpoint (`LeanOS/QotomNoSmapControl.lean`)
...
## Final Qotom PCI admission (`LeanOS/QotomPCIFinalAdmission.lean`)

The final checker binds the complete sixteen-function Command vector and five explicit platform premises. It is a conditional policy boundary; these proofs do not establish the hardware premises or physical DMA containment.

- `exported_one_iff` — The exported admission returns one exactly when the final Command vector is accepted and each of the five supplied platform premises equals one.
- `initial_trust_contract_one_iff` — The named initial trust contract returns one exactly for the accepted final Command vector; its five platform premises are fixed by the contract rather than supplied as runtime switches.

## Complete Qotom PCI inventory observations (`LeanOS/QotomPCIInventory.lean`)
...
## Qotom command and readback trace observations (`LeanOS/QotomPCIQuarantineObservation.lean`)
...
## Initial PCI inventory and command-trace binding (`LeanOS/QotomPCIQuarantineTransition.lean`)
...
- `platform_profile_admission_confined` — A successful whole-platform decision identifies one complete q35 or Qotom manifest, and every finite operation sequence preserves that identity without publishing an AP start.
...
- `qotom_entry_checkpoint_withholds_cpl3_and_gates_result` — The Qotom entry checkpoint never grants general CPL3 authority, and its frame/register claim requires the complete accepted synchronous two-entry, one-return, user-IF-disabled root-closure result.
- `qotom_blocking_ipc_authority_requires_whole_profile` — The Qotom blocking-IPC profile can report CPL3 authority only after its complete scalar platform, three-root, gate, masked-interrupt, two-context, and generated-model admission has passed.
...
- `composite_subtree_revocation_reaches_in_flight_authority` — The security-claim contract SC-COMPOSITE-INFLIGHT-REVOCATION: an accepted whole-family take-back cancels every sealed in-flight descendant atomically, leaves nothing derived from the root installed or in flight, and keeps lineage history and the identity counter unchanged.

## The one list of record names the boot images print and the checkers expect (`LeanOS/SerialProtocol.lean`)
...
## Bounded supervisor copy aliases (`LeanOS/UserCopyAliases.lean`)
...
## Copy policy and subject-page-table agreement (`LeanOS/UserCopyBinding.lean`)
...
## Validated copy-alias operands (`LeanOS/UserCopyOperands.lean`)
...
## Interrupted copy prefixes (`LeanOS/UserCopyPrefix.lean`)
...
## Copy transaction termination (`LeanOS/UserCopyTransaction.lean`)
