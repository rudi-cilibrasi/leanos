...
<!-- invariants:totals theorems=2581 files=62 examples=872 -->
...
- `compositeDispatcherTerminateSubjectTwo_allocates_retired_frame` — After canonical subject-2 termination, subject 1 can allocate never-issued object 21 in empty slot 2, and the real memory allocator deterministically binds it to the retired frame 4.
...
- `subject_termination_checked_authoritative_exact_completion_allocates_retired_frame` — The finite checked cleanup discharges those allocator premises concretely: subject 1 allocates never-issued object 21 in empty slot 2, the real allocator selects the retired frame 4, and the resulting lifetime is fresh and scrubbed without caller-supplied acceptance or binding evidence.
