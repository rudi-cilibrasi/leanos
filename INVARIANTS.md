...
<!-- invariants:totals theorems=2629 files=62 examples=872 -->
...
- `assigned_edu_reuse_fresh_mapping_validates_and_rejects_stale_generation` — The fresh cache witness uses an aligned in-range IOVA and the same generation-2 assignment, domain, mapping, and frame lifetime as a fully validated authoritative IOMMU state, where the retired generation-1 requester is denied and the independently authorized fresh request translates.
