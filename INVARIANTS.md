...
<!-- invariants:totals theorems=2635 files=63 examples=872 -->
...
- `mixedCanonicalEdges_refine` — All twenty edges of the hosted mixed corpus inherit their state and result meaning solely from that scalar-to-authoritative bridge.
...
- `Footprint.untouched_not_written` — If an operation footprint does not read a projection, the footprint discipline guarantees that it cannot write that projection either.
