...
def classify(events, expected=EXPECTED, boot_record=None, structured_terminal=True):
...
def classify_protected(events, digest, expected=EXPECTED, boot_record=None,
                       structured_terminal=True, quiet_range=(30, 90)):
    result = classify(events, expected, boot_record, structured_terminal)
    data = b''.join(bytes.fromhex(e['hex']) for e in events)
    accepted = b'LEANOS-LAB/1 WATCHDOG-WINDOW accepted=1'
    armed = b'LEANOS-LAB/1 WATCHDOG-ARMED ticks=120'
    ...
    if not quiet_range[0] <= result['quiet_seconds'] <= quiet_range[1]:
        raise ValueError('protected completion outside its observation interval')
    result['watchdog_protected'] = True
    return result


def decode_qotom_lvt_observation(data):
    prefix = b'LEANOS-LAB/1 APIC-LVT '
    pattern = re.compile(
        rb'LEANOS-LAB/1 APIC-LVT profile=qotom-lvt-v1 status=([0-9]+) '
        rb'apic-base=([0-9]+) executing=([0-9]+) lint0-first=([0-9]+) '
        rb'lint1-first=([0-9]+) lint0-second=([0-9]+) lint1-second=([0-9]+) '
        rb'stable=([0-9]+) routing-authority=([0-9]+) writes=([0-9]+) '
        rb'map-restored=([0-9]+) platform-admitted=([0-9]+)\n')
    matches = list(pattern.finditer(data))
    if len(matches) != 1 or data.count(prefix) != 1:
        raise ValueError('missing, malformed, or repeated Qotom LVT observation')
    values = [int(value) for value in matches[0].groups()]
    if any(str(value).encode() != raw for value,raw in zip(values,matches[0].groups())):
        raise ValueError('noncanonical Qotom LVT value')
    (status, apic_base, executing, lint0_first, lint1_first,
     lint0_second, lint1_second, stable, authority, writes,
     restored, admitted) = values
    if (status != 0 or apic_base != 0xfee00900 or executing != 0 or
            any(value > 0xffffffff for value in
                (lint0_first, lint1_first, lint0_second, lint1_second)) or
            stable not in (0,1) or stable != int(
                lint0_first == lint0_second and lint1_first == lint1_second) or
            authority != 0 or writes != 0 or restored != 1 or admitted != 0):
        raise ValueError('rejected Qotom LVT observation')
    def decode(value):
        return {'raw':value, 'vector':value & 0xff,
                'delivery_mode':(value >> 8) & 7,
                'delivery_status':(value >> 12) & 1,
                'polarity':(value >> 13) & 1,
                'remote_irr':(value >> 14) & 1,
                'trigger_mode':(value >> 15) & 1,
                'masked':bool((value >> 16) & 1)}
    return matches[0].group(0), {
        'profile':'qotom-lvt-v1', 'status':status, 'apic_base':apic_base,
        'executing_apic_id':executing, 'lint0_first':decode(lint0_first),
        'lint1_first':decode(lint1_first), 'lint0_second':decode(lint0_second),
        'lint1_second':decode(lint1_second), 'stable':bool(stable),
...
def classify_bsp_production(events, digest, lvt_observation=False, lvt_policy=False):
    ...
    terminal_index = markers.index(terminal)
    terminal_end = positions[terminal_index] + len(terminal)
    offset = 0
...
def cpu_diagnostic_bytes(events, protocol, handoff=False, terminal=None):
    data = b''.join(bytes.fromhex(e['hex']) for e in events)
    ...
    terminal = protocol['FINAL'].encode('ascii') if terminal is None else terminal
    final = data.find(terminal, start)
...
def cpu_replay_inputs(protocol_path, replay, pci_replay=None, handoff=False, acpi=False, pci_read_trace=False, bootstrap=False, ecam_memory=False, dsdt=False, ecam_read=False, native_inventory=False, native_kernel=False, bsp_replay=None, pci_capabilities=False, af_observation=False, ehci_capabilities=False, ehci_legacy=False, ehci_handoff=False, ehci_smi=False, ehci_operational=False, ehci_bme=False, xhci_capabilities=False, xhci_legacy=False, xhci_handoff=False, xhci_smi=False, xhci_operational=False, xhci_bme=False, pcie_device_observation=False, ahci_capabilities=False, ahci_port=False, ahci_interrupts=False, ahci_bme=False, hda_observation=False, hda_state=False, hda_bme=False, txe_status=False, rootport_bme=False, realtek_state=False, realtek_bme=False, pcie_pending=False, broadcom_d3=False, graphics_state=False, graphics_bme=False, txe_bme=False, pci_final_admission=False, pci_trust_contract=False, nosmap_control=False, copy_root_publication=False, entry_integration=False, exception_integration=False, blocking_ipc_integration=False):
    ...
    if pci_final_admission:
        result['pci_final_decoder_sha256'] = hashlib.sha256(Path(__file__).with_name('check-qotom-pci-final-capture.py').read_bytes()).hexdigest()
    if pci_trust_contract:
        result['pci_trust_contract'] = 'qotom-j1900-pci-trust-v1'
    if copy_root_publication:
        result['copy_root_publication_profile'] = 'qotom-copy-roots-v1'
        result['copy_root_publication_decoder_sha256'] = hashlib.sha256(Path(__file__).with_name('check-qotom-copy-root-publication-capture.py').read_bytes()).hexdigest()
    if entry_integration:
        result['entry_integration_profile'] = 'qotom-copy-roots-v1'
        result['entry_integration_decoder_sha256'] = hashlib.sha256(Path(__file__).with_name('check-qotom-entry-integration-capture.py').read_bytes()).hexdigest()
    ...
    if txe_bme:
        result['txe_bme_decoder_sha256'] = hashlib.sha256(Path(__file__).with_name('check-qotom-txe-bme-capture.py').read_bytes()).hexdigest()
    if graphics_bme:
        result['graphics_bme_decoder_sha256'] = hashlib.sha256(Path(__file__).with_name('check-qotom-graphics-bme-capture.py').read_bytes()).hexdigest()
    if graphics_state:
        result['graphics_state_decoder_sha256'] = hashlib.sha256(Path(__file__).with_name('check-qotom-graphics-state-capture.py').read_bytes()).hexdigest()
    if broadcom_d3:
        result['broadcom_d3_decoder_sha256'] = hashlib.sha256(Path(__file__).with_name('check-qotom-broadcom-d3-capture.py').read_bytes()).hexdigest()
    if pcie_pending:
        result['pcie_pending_decoder_sha256'] = hashlib.sha256(Path(__file__).with_name('check-qotom-pcie-pending-capture.py').read_bytes()).hexdigest()
    if realtek_bme:
...
def classify_cpu_protected(events, digest, protocol_path, replay, pci_replay=None, handoff=False, acpi=False, pci_read_trace=False, bootstrap=False, ecam_memory=False, dsdt=False, ecam_read=False, native_inventory=False, native_kernel=False, bsp_replay=None, pci_capabilities=False, af_observation=False, ehci_capabilities=False, ehci_legacy=False, ehci_handoff=False, ehci_smi=False, ehci_operational=False, ehci_bme=False, xhci_capabilities=False, xhci_legacy=False, xhci_handoff=False, xhci_smi=False, xhci_operational=False, xhci_bme=False, pcie_device_observation=False, ahci_capabilities=False, ahci_port=False, ahci_interrupts=False, ahci_bme=False, hda_observation=False, hda_state=False, hda_bme=False, txe_status=False, rootport_bme=False, realtek_state=False, realtek_bme=False, pcie_pending=False, broadcom_d3=False, graphics_state=False, graphics_bme=False, txe_bme=False, pci_final_admission=False, pci_trust_contract=False, nosmap_control=False, copy_root_publication=False, entry_integration=False, exception_integration=False, blocking_ipc_integration=False):
    ...
    if copy_root_publication and not nosmap_control:
        raise ValueError('--copy-root-publication requires --nosmap-control')
    if entry_integration and not copy_root_publication:
    ...
    if txe_bme and not graphics_bme:
        raise ValueError('--txe-bme requires --graphics-bme')
    if graphics_bme and not graphics_state:
        raise ValueError('--graphics-bme requires --graphics-state')
    if graphics_state and not broadcom_d3:
        raise ValueError('--graphics-state requires --broadcom-d3')
    if broadcom_d3 and not pcie_pending:
        raise ValueError('--broadcom-d3 requires --pcie-pending')
    if pcie_pending and not realtek_bme:
        raise ValueError('--pcie-pending requires --realtek-bme')
    ...
    if blocking_ipc_integration:
        ...
        decoder = runpy.run_path(str(Path(__file__).with_name('check-qotom-entry-integration-capture.py')))
        raw, result['entry_integration'] = decoder['extract'](raw, protocol)
    if copy_root_publication:
        decoder = runpy.run_path(str(Path(__file__).with_name('check-qotom-copy-root-publication-capture.py')))
        raw, result['copy_root_publication'] = decoder['extract'](raw, protocol)
        if (entry_integration and result['entry_integration']['closed_root'] !=
                result['copy_root_publication']['closed_root']):
            raise ValueError('entry used a different closed root than publication')
    if nosmap_control:
        decoder = runpy.run_path(str(Path(__file__).with_name('check-qotom-nosmap-control-capture.py')))
        raw, result['nosmap_control'] = decoder['extract'](raw, protocol)
    if pci_final_admission:
        decoder = runpy.run_path(str(Path(__file__).with_name('check-qotom-pci-final-capture.py')))
        raw, result['pci_final'] = decoder['extract'](raw, protocol, trust_contract=pci_trust_contract)
    if txe_bme:
        decoder = runpy.run_path(str(Path(__file__).with_name('check-qotom-txe-bme-capture.py')))
        raw, result['txe_bme'] = decoder['extract'](raw, protocol)
    if graphics_bme:
        decoder = runpy.run_path(str(Path(__file__).with_name('check-qotom-graphics-bme-capture.py')))
        raw, result['graphics_bme'] = decoder['extract'](raw, protocol)
    if graphics_state:
        decoder = runpy.run_path(str(Path(__file__).with_name('check-qotom-graphics-state-capture.py')))
        raw, result['graphics_state'] = decoder['extract'](raw, protocol)
    if broadcom_d3:
        decoder = runpy.run_path(str(Path(__file__).with_name('check-qotom-broadcom-d3-capture.py')))
        raw, result['broadcom_d3'] = decoder['extract'](raw, protocol)
    if pcie_pending:
        decoder = runpy.run_path(str(Path(__file__).with_name('check-qotom-pcie-pending-capture.py')))
        raw, result['pcie_pending'] = decoder['extract'](raw, protocol)
    if realtek_bme:
    ...
    if pcie_pending and result['pcie_pending'] is not None:
        diagnostic['terminal_reason'] = result['pcie_pending']['terminal_reason']
        diagnostic['replay_scope'] = 'native-inventory-with-pcie-nonposted-quiet-observation'
    if broadcom_d3 and result['broadcom_d3'] is not None:
        diagnostic['terminal_reason'] = result['broadcom_d3']['terminal_reason']
        diagnostic['replay_scope'] = 'native-inventory-with-broadcom-d3hot-observation'
    if graphics_state and result['graphics_state'] is not None:
        diagnostic['terminal_reason'] = result['graphics_state']['terminal_reason']
        diagnostic['replay_scope'] = 'native-inventory-with-graphics-state-observation'
    if graphics_bme and result['graphics_bme'] is not None:
        diagnostic['terminal_reason'] = result['graphics_bme']['terminal_reason']
        diagnostic['replay_scope'] = 'native-inventory-with-graphics-bme-transition'
    if txe_bme and result['txe_bme'] is not None:
        diagnostic['terminal_reason'] = result['txe_bme']['terminal_reason']
        diagnostic['replay_scope'] = 'native-inventory-with-txe-host-bme-transition'
    if pci_final_admission and result['pci_final'] is not None:
        diagnostic['terminal_reason'] = result['pci_final']['terminal_reason']
        diagnostic['replay_scope'] = ('qotom-initial-pci-trust-contract' if pci_trust_contract else
                                      'qotom-final-pci-observation-with-unmet-assumptions')
    if nosmap_control and result['nosmap_control'] is not None:
        diagnostic['terminal_reason'] = result['nosmap_control']['terminal_reason']
        diagnostic['replay_scope'] = 'qotom-nosmap-control-transition'
    if copy_root_publication and result['copy_root_publication'] is not None:
        diagnostic['terminal_reason'] = result['copy_root_publication']['terminal_reason']
        diagnostic['replay_scope'] = 'qotom-closed-and-copy-root-publication'
...
def main():
    ...
    parser.add_argument('--pcie-pending', action='store_true')
    parser.add_argument('--broadcom-d3', action='store_true')
    parser.add_argument('--graphics-state', action='store_true')
    parser.add_argument('--graphics-bme', action='store_true')
    parser.add_argument('--txe-bme', action='store_true')
    parser.add_argument('--pci-final-admission', action='store_true')
    parser.add_argument('--pci-trust-contract', action='store_true')
    parser.add_argument('--nosmap-control', action='store_true')
    ...
    if args.pci_final_admission and not args.txe_bme:
        parser.error('--pci-final-admission requires --txe-bme')
    if args.pci_trust_contract and not args.pci_final_admission:
        parser.error('--pci-trust-contract requires --pci-final-admission')
    if args.nosmap_control and not args.pci_trust_contract:
        parser.error('--nosmap-control requires --pci-trust-contract')
    if args.copy_root_publication and not args.nosmap_control:
        parser.error('--copy-root-publication requires --nosmap-control')
    if args.entry_integration and not args.copy_root_publication:
    ...
    if args.graphics_bme and not args.graphics_state:
        parser.error('--graphics-bme requires --graphics-state')
    if args.graphics_state and not args.broadcom_d3:
        parser.error('--graphics-state requires --broadcom-d3')
    if args.broadcom_d3 and not args.pcie_pending:
        parser.error('--broadcom-d3 requires --pcie-pending')
    if args.pcie_pending and not args.realtek_bme:
        parser.error('--pcie-pending requires --realtek-bme')
        ...
        if args.scenario == 'watchdog-kernel':
            ...
            if args.bsp_production:
                ...
                if args.blocking_ipc_integration:
                    ...
                    (directory / 'entry-integration.json').write_text(json.dumps(entry_metadata, indent=2) + '\n')
                elif args.entry_integration:
                    decoder = runpy.run_path(str(Path(__file__).with_name('check-qotom-entry-integration-capture.py')))
                    raw, entry_metadata = decoder['extract'](raw, protocol)
                    (directory / 'entry-integration.json').write_text(json.dumps(entry_metadata, indent=2) + '\n')
                if args.copy_root_publication:
                    decoder = runpy.run_path(str(Path(__file__).with_name('check-qotom-copy-root-publication-capture.py')))
                    raw, copy_root_metadata = decoder['extract'](raw, protocol)
                    if (args.entry_integration and entry_metadata['closed_root'] !=
                            copy_root_metadata['closed_root']):
                        raise ValueError('entry used a different closed root than publication')
                    (directory / 'copy-root-publication.json').write_text(json.dumps(copy_root_metadata, indent=2) + '\n')
                if args.nosmap_control:
                    decoder = runpy.run_path(str(Path(__file__).with_name('check-qotom-nosmap-control-capture.py')))
                    raw, nosmap_metadata = decoder['extract'](raw, protocol)
                    (directory / 'nosmap-control.json').write_text(json.dumps(nosmap_metadata, indent=2) + '\n')
                if args.pci_final_admission:
                    decoder = runpy.run_path(str(Path(__file__).with_name('check-qotom-pci-final-capture.py')))
                    raw, pci_final_metadata = decoder['extract'](raw, protocol, trust_contract=args.pci_trust_contract)
                    (directory / 'pci-final.json').write_text(json.dumps(pci_final_metadata, indent=2) + '\n')
                if args.txe_bme:
                    decoder = runpy.run_path(str(Path(__file__).with_name('check-qotom-txe-bme-capture.py')))
                    raw, txe_bme_metadata = decoder['extract'](raw, protocol)
                    (directory / 'txe-bme.json').write_text(json.dumps(txe_bme_metadata, indent=2) + '\n')
                if args.graphics_bme:
                    decoder = runpy.run_path(str(Path(__file__).with_name('check-qotom-graphics-bme-capture.py')))
                    raw, graphics_bme_metadata = decoder['extract'](raw, protocol)
                    (directory / 'graphics-bme.json').write_text(json.dumps(graphics_bme_metadata, indent=2) + '\n')
                if args.graphics_state:
                    decoder = runpy.run_path(str(Path(__file__).with_name('check-qotom-graphics-state-capture.py')))
                    raw, graphics_state_metadata = decoder['extract'](raw, protocol)
                    (directory / 'graphics-state.json').write_text(json.dumps(graphics_state_metadata, indent=2) + '\n')
                if args.broadcom_d3:
                    decoder = runpy.run_path(str(Path(__file__).with_name('check-qotom-broadcom-d3-capture.py')))
                    raw, broadcom_d3_metadata = decoder['extract'](raw, protocol)
                    (directory / 'broadcom-d3.json').write_text(json.dumps(broadcom_d3_metadata, indent=2) + '\n')
                if args.pcie_pending:
                    decoder = runpy.run_path(str(Path(__file__).with_name('check-qotom-pcie-pending-capture.py')))
                    raw, pcie_pending_metadata = decoder['extract'](raw, protocol)
                    (directory / 'pcie-pending.json').write_text(json.dumps(pcie_pending_metadata, indent=2) + '\n')
                if args.realtek_bme:
                    decoder = runpy.run_path(str(Path(__file__).with_name('check-qotom-realtek-bme-capture.py')))
