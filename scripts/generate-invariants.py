...
promise machine-checked:
  ...
  decompose Split the rendered document into checked-in per-module summaries.
  render    Rebuild the document byte-for-byte from those checked-in inputs.
...
DEFAULT_SECTIONS = REPO_ROOT / "INVARIANTS.sections"
...
DOC_SECTION_HEADING_RE = re.compile(
    r"^## (?P<title>.+) \(`(?P<source>[^`]+\.lean)`\)$", re.MULTILINE
)
PROVENANCE_RE = re.compile(
    r"Generated by model `(?P<generator>[^`]+)` on "
    r"(?P<date>\d{4}-\d{2}-\d{2}) with"
)
...
def cmd_rename(args):
    ...
    decompose_checked_in(per_file, text, Path(args.sections_dir))
...
def decompose_checked_in(per_file, document, output):
    matches = list(DOC_SECTION_HEADING_RE.finditer(document))
    found = [match.group("source") for match in matches]
    if Counter(found) != Counter(per_file.keys()):
        raise ValueError("document_sections_do_not_match_sources")
    provenance = PROVENANCE_RE.search(document)
    if provenance is None:
        raise ValueError("document_provenance_missing")
    output.mkdir(parents=True, exist_ok=True)
    for index, match in enumerate(matches):
        rel_path = match.group("source")
        end = matches[index + 1].start() if index + 1 < len(matches) else len(document)
        body = document[match.end():end].strip()
        section = f"# {match.group('title')}\n\n{body}\n"
        parse_section(section, rel_path, per_file[rel_path])
        (output / section_key(rel_path)).write_text(section, encoding="utf-8")
    manifest = {
        "schemaVersion": 1,
        "generator": provenance.group("generator"),
        "date": provenance.group("date"),
...
def cmd_decompose(args):
    """Split the rendered document into checked-in deterministic sections."""

    per_file, _ = extract_all()
    document = Path(args.document).read_text(encoding="utf-8")
    output = Path(args.sections_dir)
    count = decompose_checked_in(per_file, document, output)
    print(f"wrote {count} deterministic sections -> {output}")
    return 0


def render_checked_in(per_file, total_examples, sections_dir):
    sections_dir = Path(sections_dir)
    manifest = json.loads(
        (sections_dir / "manifest.json").read_text(encoding="utf-8")
    )
    if manifest.get("schemaVersion") != 1:
        raise ValueError("unsupported_sections_manifest")
    if manifest.get("files") != sorted(per_file):
        raise ValueError("sections_manifest_diverges_from_sources")
    sections = load_sections(per_file, sections_dir)
...
def cmd_render(args):
    per_file, total_examples = extract_all()
    document = render_checked_in(per_file, total_examples, args.sections_dir)
    Path(args.output).write_text(document, encoding="utf-8")
    print(f"wrote {args.output} from checked-in sections")
    return 0
...
def cmd_verify(args):
    ...
    errors = []
    try:
        rendered = render_checked_in(per_file, total_examples, args.sections_dir)
    except (ValueError, OSError, KeyError, json.JSONDecodeError) as err:
        errors.append(f"checked-in sections are invalid: {err}")
    else:
        if rendered != doc:
            errors.append(
                "INVARIANTS.md differs from deterministic checked-in sections; "
...
def main():
    ...
    p.add_argument("--sections-dir", default=str(DEFAULT_SECTIONS))
    ...
    p = sub.add_parser(
        "decompose", help="split INVARIANTS.md into deterministic section inputs"
    )
    p.add_argument("--document", default=str(DEFAULT_DOC))
    p.add_argument("--sections-dir", default=str(DEFAULT_SECTIONS))
    ...
    p = sub.add_parser(
        "render", help="rebuild INVARIANTS.md from checked-in section inputs"
    )
    p.add_argument("--sections-dir", default=str(DEFAULT_SECTIONS))
