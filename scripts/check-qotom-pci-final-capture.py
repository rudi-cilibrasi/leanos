...
def extract(raw,protocol):
    ...
    assumptions=protocol['FINAL'].encode()+b' status=FAIL reason=qotom-pci-assumptions\n'
    ...
    pattern=(PREFIX+rb'profile=qotom-pci-final-v1 status='+DEC+rb' index='+DEC+
        ...
        rb' admitted='+DEC+rb' commands='+rb'([0-9]+(?:,[0-9]+){15})'+
        rb' vtd=not-applicable platform-admitted=0\n')
    ...
    commands=tuple(map(int,match.group(7).split(b',')))
    if (status,index,count,commands_accepted,assumption_mask,admitted)!=(6,16,16,1,3,0) or commands!=COMMANDS:
    ...
    return projection,{
        ...
        'platform_admitted':False,'terminal_reason':'qotom-pci-assumptions',
