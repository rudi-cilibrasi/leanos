...
tmp="$(mktemp -d)"
trap 'rm -rf "$tmp"' EXIT
evidence="${LEANOS_INFLIGHT_REVOCATION_NEGATIVE_EVIDENCE_DIR:-build/boot/inflight-revocation-negatives}"
fixture_timeout="${LEANOS_INFLIGHT_REVOCATION_FIXTURE_TIMEOUT_SECONDS:-5}"
mkdir -p "$evidence"
...
invoke() {
  LEANOS_BOOT_SCENARIO=inflight-revocation \
    LEANOS_ORACLE_CORPUS="$tmp/oracle/corpus.tsv" \
    LEANOS_QEMU="$root/tests/qemu-inflight-revocation-fixture.sh" \
    LEANOS_INFLIGHT_REVOCATION_FIXTURE_MODE="$1" \
...
for spec in \
  'envelope-only serial-protocol' \
  'pending-only serial-protocol' \
  'installed-after-revoke serial-protocol' \
  'delivered-handle serial-protocol' \
  'old-identity-reused serial-protocol' \
  'truncated-generation serial-protocol' \
  'caller-chosen-root serial-protocol' \
  'stale-b-context serial-protocol' \
  'old-adapter serial-protocol' \
  'state-splice serial-protocol' \
  'replayed-offer serial-protocol' \
...
echo "In-flight revocation runner success and 16 controlled negatives passed"
