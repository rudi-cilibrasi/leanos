...
invoke() { LEANOS_BOOT_SCENARIO=preemption LEANOS_ORACLE_CORPUS="$tmp/oracle/corpus.tsv" LEANOS_QEMU="$root/tests/qemu-fixture.sh" LEANOS_QEMU_FIXTURE_MODE="$1" LEANOS_QEMU_TIMEOUT_SECONDS=5 LEANOS_SERIAL_LOG="$tmp/$1.serial" LEANOS_DMA_SNAPSHOT="$tmp/$1.dma.tsv" LEANOS_SOURCE_REVISION_FILE="$tmp/SOURCE_REVISION" ./scripts/run-image.sh "$tmp/image.iso"; }
invoke success >/dev/null 2>&1
for spec in 'entry-high-water-missing entry-stack-high-water' 'entry-high-water-invalid entry-stack-high-water' 'entry-high-water-duplicate entry-stack-high-water' 'entry-high-water-reordered entry-stack-high-water' 'entry-high-water-wrong-path entry-stack-high-water' 'interrupts-disabled serial-protocol' 'timer-missing serial-protocol' 'missing-second-tick serial-protocol' 'old-resumed serial-protocol' 'fresh-restart serial-protocol' 'cross-restored serial-protocol' 'stale-resume-cr3 serial-protocol' 'wrong-binding serial-protocol' 'duplicate-tick serial-protocol' 'corrupt-canary serial-protocol' 'corrupt-stack serial-protocol' 'corrupt-flags serial-protocol' 'corrupt-selectors serial-protocol'; do read -r mode class <<< "$spec"; set +e; invoke "$mode" >"$tmp/$mode.output" 2>&1; status=$?; set -e; [[ $status -ne 0 ]] && grep -q "failure_class=$class" "$tmp/$mode.output" || { cat "$tmp/$mode.output" >&2; exit 1; }; done
