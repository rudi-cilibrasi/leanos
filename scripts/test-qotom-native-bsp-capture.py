...
class Capture(unittest.TestCase):
    ...
    def test_realtek_bme_protected_projection(self):
        capture=ROOT / 'hardware/lab/observations/qotom-native-realtek-state-20260911'
        expected=json.loads((capture / 'cycle-1/result.json').read_text())
        events=[json.loads(line) for line in (capture / 'cycle-1/events.jsonl').read_text().splitlines()]
        raw=b''.join(bytes.fromhex(e['hex']) for e in events)
        end=raw.index(FINAL)+len(FINAL)
        def record(index=13,status=0,values=(1,7,3)):
            return (f'LEANOS-LAB/1 REALTEK-BME profile=qotom-realtek-bme-v1 index={index} status={status}' +
        ...
        success=record()+record(15)
        result=check(success)
        self.assertEqual([f['index'] for f in result['realtek_bme']['functions']],[13,15])
        self.assertEqual(result['diagnostic']['replay_scope'],'native-inventory-with-realtek-bme-observation')
        self.assertIn('realtek_bme_decoder_sha256',result['diagnostic'])
        ...
        failure=FINAL.replace(b'qotom-platform-pending',b'qotom-realtek-bme')
        failures=((3,(0,0,0)),(4,(0,0,0)),(5,(0,0,0)),(6,(1,7,0)),
            (7,(1,7,0)),(8,(1,7,3)),(9,(0,0,0)),(10,(0,0,0)))
        for index in (13,15):
            prefix=record() if index==15 else b''
            for status,values in failures:
                payload=prefix+record(index,status,values)
                result=check(payload,failure)
                self.assertEqual(result['diagnostic']['terminal_reason'],'qotom-realtek-bme')
                self.assertEqual(result['realtek_bme']['functions'][-1]['status'],status)
                with self.assertRaises(ValueError):check(payload)
                with self.assertRaises(ValueError):check(payload+record(index+2),failure)
        invalid=(b'',record(),record(15)+record(),success+record(15),
            success.replace(b'index=13',b'index=14'),success.replace(b'index=13',b'index=013'),
            success.replace(b'status=0',b'status=1',1),success.replace(b'status=0',b'status=2',1),
            success.replace(b'status=0',b'status=11',1),success.replace(b'attempted=1',b'attempted=2',1),
            success.replace(b'before=7',b'before=6',1),success.replace(b'after=3',b'after=7',1),
            record(13,3,(1,7,0)),record(13,6,(0,0,0)),record(13,7,(1,7,3)),
            record(13,9,(1,7,0)))
        for payload in invalid:
            with self.assertRaises(ValueError):check(payload)
        with self.assertRaises(ValueError):check(success,failure)
        with self.assertRaises(ValueError):
            R['classify_cpu_protected'](events,expected['elf_sha256'],
                capture / 'diagnostic-protocol.tsv',CPU,PCI,realtek_bme=True)

    def test_pcie_pending_protected_projection(self):
        capture=ROOT / 'hardware/lab/observations/qotom-native-realtek-bme-20260911'
        expected=json.loads((capture / 'cycle-1/result.json').read_text())
        events=[json.loads(line) for line in (capture / 'cycle-1/events.jsonl').read_text().splitlines()]
        raw=b''.join(bytes.fromhex(e['hex']) for e in events)
        end=raw.index(FINAL)+len(FINAL)
        indices=(6,7,8,9,13,15);base=(17,17,17,16,25,25)
        def record(slot,status=0,polls=2,device_status=None):
            if device_status is None:device_status=base[slot]
            return (f'LEANOS-LAB/1 PCIE-PENDING profile=qotom-pcie-pending-v1 index={indices[slot]}' +
                f' status={status} polls={polls} device-status={device_status}\n').encode()
        def check(records,terminal=FINAL):
            changed=raw[:end].replace(FINAL,records+terminal)
            synthetic=[{'elapsed':0,'hex':changed.hex()},{'elapsed':37,'hex':raw[end:].hex()}]
            return R['classify_cpu_protected'](synthetic,expected['elf_sha256'],
                ...
                hda_bme=True,txe_status=True,rootport_bme=True,realtek_state=True,realtek_bme=True,
                pcie_pending=True)
        success=b''.join(record(slot) for slot in range(6))
        result=check(success)
        self.assertEqual([f['index'] for f in result['pcie_pending']['functions']],list(indices))
        self.assertTrue(result['pcie_pending']['nonposted_quiet_observed'])
        self.assertFalse(result['pcie_pending']['posted_write_drain_established'])
        self.assertFalse(result['pcie_pending']['transaction_drain_established'])
        self.assertIn('pcie_pending_decoder_sha256',result['diagnostic'])
        self.assertEqual(result['diagnostic']['replay_scope'],
            'native-inventory-with-pcie-nonposted-quiet-observation')
        failure=FINAL.replace(b'qotom-platform-pending',b'qotom-pcie-pending')
        cases=((2,0,0),(3,0,0),(4,0,0),(5,0,0),(6,1,None),(7,100,'pending'),(8,0,0),(9,0,0))
        for slot in range(6):
            prefix=b''.join(record(i) for i in range(slot))
            for status,polls,device in cases:
                value=base[slot]|0x20 if device=='pending' else (base[slot] if device is None else device)
                payload=prefix+record(slot,status,polls,value)
                result=check(payload,failure)
                self.assertEqual(result['diagnostic']['terminal_reason'],'qotom-pcie-pending')
                self.assertEqual(result['pcie_pending']['functions'][-1]['status'],status)
                with self.assertRaises(ValueError):check(payload)
                with self.assertRaises(ValueError):check(payload+record(min(slot+1,5)),failure)
        invalid=(b'',record(0),record(1)+record(0),success+record(5),
            success.replace(b'index=6',b'index=5',1),success.replace(b'polls=2',b'polls=1',1),
            success.replace(b'device-status=17',b'device-status=49',1),
            success.replace(record(0),record(0,1),1),record(0,6,0,0),record(0,7,99,49))
        for payload in invalid:
            with self.assertRaises(ValueError):check(payload)
        with self.assertRaises(ValueError):check(success,failure)
        with self.assertRaises(ValueError):
    ...
    def test_graphics_state_protected_projection(self):
        capture=ROOT / 'hardware/lab/observations/qotom-native-broadcom-d3-20260912'
        expected=json.loads((capture / 'cycle-1/result.json').read_text())
        events=[json.loads(line) for line in (capture / 'cycle-1/events.jsonl').read_text().splitlines()]
        raw=b''.join(bytes.fromhex(e['hex']) for e in events)
        end=raw.index(FINAL)+len(FINAL)
        words=([0,0,0,0,0x200]*3)*2
        record=('LEANOS-LAB/1 GRAPHICS-STATE profile=qotom-valleyview-rings-v1 index=1 '
            'status=0 width=30 words='+','.join(map(str,words))+'\n').encode()
        ...
        result=R['classify_cpu_protected'](synthetic,expected['elf_sha256'],
            ...
            ahci_port=True,ahci_interrupts=True,ahci_bme=True,hda_observation=True,hda_state=True,
            hda_bme=True,txe_status=True,rootport_bme=True,realtek_state=True,realtek_bme=True,
            pcie_pending=True,broadcom_d3=True,graphics_state=True)
        self.assertTrue(result['graphics_state']['stable'])
        self.assertTrue(result['graphics_state']['rings_invalid_and_idle'])
        self.assertTrue(result['graphics_state']['rings_empty'])
        self.assertTrue(result['graphics_state']['display_decode_preserved'])
        self.assertTrue(result['graphics_state']['graphics_bme_preserved'])
    ...
    def test_txe_bme_protected_projection(self):
        ...
        publication_record=(b'LEANOS-LAB/1 COPY-ROOTS profile=qotom-copy-roots-v1 '
            b'status=0 protected=5 removed-aliases=3 retained-present=4088 aliases=2 '
        ...
        entry_terminal=FINAL.replace(b'qotom-platform-pending',b'qotom-entry-integration-pending')
        publication_changed=changed.replace(
            FINAL,trust_record+control_record+publication_record+entry_terminal)
        publication_events=[{'elapsed':0,'hex':publication_changed.hex()},
                            {'elapsed':37,'hex':raw[end:].hex()}]
        result=R['classify_cpu_protected'](publication_events,expected['elf_sha256'],
            capture / 'diagnostic-protocol.tsv',CPU,PCI,handoff=True,acpi=True,
            bootstrap=True,ecam_memory=True,dsdt=True,ecam_read=True,
            native_inventory=True,native_kernel=True,bsp_replay=BSP,
            pci_capabilities=True,af_observation=True,ehci_capabilities=True,
            ...
            pcie_pending=True,broadcom_d3=True,graphics_state=True,graphics_bme=True,txe_bme=True,
            pci_final_admission=True,pci_trust_contract=True,nosmap_control=True,
            copy_root_publication=True)
        self.assertEqual(result['copy_root_publication']['transferred'],16)
        self.assertTrue(result['copy_root_publication']['bytes_match'])
        self.assertEqual(result['diagnostic']['terminal_reason'],'qotom-entry-integration-pending')
        self.assertEqual(result['diagnostic']['replay_scope'],
                         'qotom-closed-and-copy-root-publication')
    ...
    def test_retained_copy_root_publication_capture(self):
        capture=ROOT / 'hardware/lab/observations/qotom-native-copy-root-publication-20260912'
        manifest=json.loads((capture / 'manifest.json').read_text())
        for name,digest in manifest['files'].items():
            self.assertEqual(hashlib.sha256((capture / name).read_bytes()).hexdigest(),digest)
        expected=json.loads((capture / 'cycle-1/result.json').read_text())
        ...
        result=R['classify_cpu_protected'](events,expected['elf_sha256'],
            ...
            pci_final_admission=True,pci_trust_contract=True,nosmap_control=True,
            copy_root_publication=True)
        self.assertEqual(result['copy_root_publication'],expected['copy_root_publication'])
        self.assertEqual(result['diagnostic']['terminal_reason'],'qotom-entry-integration-pending')
        self.assertEqual(result['quiet_seconds'],expected['quiet_seconds'])
        self.assertTrue(expected['request_consumed'])
        self.assertNotEqual(expected['freebsd_boot_before'],expected['freebsd_boot_after'])
        publication=result['copy_root_publication']
        self.assertEqual(publication['transferred'],16)
        self.assertTrue(publication['bytes_match'])
        self.assertEqual(publication['active_root'],publication['closed_root'])
        self.assertTrue(publication['closed_root_published'])
        self.assertTrue(publication['copy_root_published'])
        self.assertFalse(publication['cpl3_authority'])
    ...
    def test_retained_realtek_state_capture(self):
        capture = ROOT / 'hardware/lab/observations/qotom-native-realtek-state-20260911'
        manifest = json.loads((capture / 'manifest.json').read_text())
        for name,digest in manifest['files'].items():
            self.assertEqual(hashlib.sha256((capture / name).read_bytes()).hexdigest(),digest)
        ...
        actual = [
            dict(index=index,status=0,transmit_before=0x2f900d00,
                command_before=0,interrupt_mask=0,receive=0x0002ff0e,
                command_after=0,transmit_after=0x2f900d00)
            for index in (13,15)]
        self.assertEqual(result['realtek_state']['functions'],actual)
        self.assertFalse(result['realtek_state']['dma_quarantine_established'])
    ...
    def test_retained_pcie_pending_capture(self):
        ...
        result = R['classify_cpu_protected'](events,expected['elf_sha256'],
            ...
            hda_bme=True,txe_status=True,rootport_bme=True,realtek_state=True,realtek_bme=True,
            pcie_pending=True)
        self.assertEqual(result['pcie_pending'],expected['pcie_pending'])
        self.assertEqual(result['diagnostic']['terminal_reason'],'qotom-platform-pending')
        self.assertEqual(result['diagnostic']['replay_scope'],
            'native-inventory-with-pcie-nonposted-quiet-observation')
        self.assertEqual(result['quiet_seconds'],expected['quiet_seconds'])
        self.assertTrue(expected['request_consumed'])
        self.assertNotEqual(expected['freebsd_boot_before'],expected['freebsd_boot_after'])
        self.assertEqual(result['pcie_pending']['functions'],[
            dict(index=index,status=0,polls=2,device_status=status,transactions_pending=False)
            for index,status in zip((6,7,8,9,13,15),(17,17,17,16,25,25))])
        self.assertTrue(result['pcie_pending']['nonposted_quiet_observed'])
        self.assertFalse(result['pcie_pending']['posted_write_drain_established'])
        self.assertFalse(result['pcie_pending']['transaction_drain_established'])
        self.assertFalse(result['pcie_pending']['dma_quarantine_established'])
        self.assertFalse(result['pcie_pending']['firmware_exclusion_established'])
