...
for spec in 'missing boot-allocation-trace' 'partial boot-allocation-trace' 'missing-scrub boot-allocation-trace' 'wrong-memory-map boot-allocation-trace' 'reordered-allocation boot-allocation-trace' 'missing-paging page-table-fixtures' 'omit-block serial-protocol' 'old-handoff serial-protocol' 'wrong-context serial-protocol' 'missing-wake serial-protocol' 'duplicate-wake serial-protocol' 'stolen-delivery serial-protocol' 'forged-pass serial-protocol' 'guest-error guest-error' 'hang timeout'; do read -r mode class <<< "$spec"; set +e; invoke "$mode" >"$tmp/$mode.output" 2>&1; status=$?; set -e; [[ $status -ne 0 ]] && grep -q "failure_class=$class" "$tmp/$mode.output" && [[ -f "$tmp/$mode.serial" ]] || { cat "$tmp/$mode.output" >&2; exit 1; }; done
