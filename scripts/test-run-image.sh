...
[[ $(grep -c '^0:' "$tmp/success.dma.tsv") -eq 6 ]] &&
  ...
  grep -Fxq $'meta\tglobal-composite-state\t257' "$tmp/success.dma.tsv" &&
  grep -Fxq $'meta\tgenerated-policy-result\t65793' "$tmp/success.dma.tsv" &&
