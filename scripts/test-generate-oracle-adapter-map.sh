...
if printf 'adapter-id\tKernelTransition\t0\tleanos_boot_transition\t2\n0\tmissing-value\tKernelTransition\t0,0\t1\n' |
    awk -f "$root/scripts/render-oracle-header.awk" > /dev/null 2> "$tmp/error"; then
  echo "error: oracle row without a value result word was accepted" >&2
  exit 1
fi
grep -Fxq 'error: malformed generated oracle result words: missing-value' "$tmp/error"
