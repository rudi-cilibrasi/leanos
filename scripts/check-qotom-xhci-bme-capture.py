...
D = runpy.run_path(str(Path(__file__).with_name('check-qotom-xhci-operational-capture.py')))
PREFIX = b'LEANOS-LAB/1 XHCI-BME '
...
def extract(raw, protocol):
    ...
    failure = protocol['FINAL'].encode() + b' status=FAIL reason=qotom-xhci-bme\n'
    if not indices:
    ...
    match = re.fullmatch(PREFIX + rb'profile=qotom-xhci-bme-v1 index=3 status=' + DEC +
    ...
    if status > 12 or status in (1,2) or attempted > 1 or max(before,after) > 65535:
        raise ValueError('BME scalar bounds or impossible native status')
    if status < 9:
        if tuple(previous[k] for k in ('status_before','command','status_after')) != (1,0,1):
    ...
    return projection, {'schema':'leanos-qotom-xhci-bme-observation-v1',
        'status':status,'write_attempted':attempted,'before_command':before,'after_command':after,
        'terminal_reason':'qotom-xhci-bme' if status else 'qotom-platform-pending',
