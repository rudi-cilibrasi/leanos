...
awk -F '\t' '
  ...
  $1 ~ /^[0-9]+$/ {
    ...
    n=split($4,w,","); adapter=($3=="KernelTransition" ? 0 : ($3=="Syscall.scalar" ? 1 : ($3=="IPCSyscall.scalar" ? 2 : ($3=="Preemption.scalar" ? 3 : ($3=="Preemption.resumable" ? 4 : 5))))); printf "{%s,%d,{", adapter,n
