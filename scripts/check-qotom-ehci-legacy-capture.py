"""Validate an EHCI extended list against its preceding MMIO pointer."""
import re
import runpy
from pathlib import Path
D = runpy.run_path(str(Path(__file__).with_name('check-qotom-ehci-capture.py')))
SUMMARY = b'LEANOS-LAB/1 EHCI-LEGACY '
ENTRY = b'LEANOS-LAB/1 EHCI-EXT '
DEC = rb'(0|[1-9][0-9]{0,9})'


def extract(raw, protocol):
    if len(raw) > 131072 or not raw.endswith(b'\n'):
        raise ValueError('EHCI legacy capture bounds')
    ...
    failure = protocol['FINAL'].encode() + b' status=FAIL reason=qotom-ehci-legacy\n'
    if not indices:
    ...
    if caps is None or caps['status']:
        raise ValueError('legacy list without complete EHCI capability observation')
    match = re.fullmatch(SUMMARY + rb'profile=qotom-legacy-v1 index=10 status=' + DEC +
    ...
    if status > 11 or status == 1 or count > 48 or offset > 255 or control > 0xffffffff:
    ...
    if status:
        ...
        expected=(caps['capability']>>8)&255
        seen=set()
        legacy=0
        for i in range(count):
            ...
            if (address!=expected or address<64 or address>252 or address&3 or address in seen or
                    value>0xffffffff or value&255 in (0,255)):
                raise ValueError('extended link bounds, cycle or ID')
            seen.add(address)
            if value&255==1:
                if legacy or address>248:
    ...
    return projection, {'schema':'leanos-qotom-ehci-legacy-observation-v1',
        'status':status,'headers':headers,'legacy_offset':offset,'control_status':control,
        'terminal_reason':'qotom-ehci-legacy' if status else 'qotom-platform-pending',
        'failed_reads_replayed':False,'ownership_established':False,'dma_quarantine_established':False}
