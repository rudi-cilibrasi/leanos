...
if [[ "$scenario" == fast-entry-syscall || "$scenario" == fast-entry-sysenter ]]; then
  echo "${LEANOS_SERIAL_14_BOOT} target=x86_64-q35 subjects=2 schedule=fast-entry-denial controls=wp,smep,smap,em,mp,ts,sce-off" > "$expected"
  ...
  echo "${LEANOS_SERIAL_13_BOOT} target=x86_64-q35 subjects=2 schedule=extended-state-denial controls=wp,smep,smap,em,mp,ts" > "$expected"
elif [[ "$scenario" == preemption ]]; then
  echo "${LEANOS_SERIAL_6_BOOT} target=x86_64-q35 subjects=2 schedule=bounded-two-shot-pit controls=wp,smep,smap" > "$expected"
elif [[ "$scenario" == stale-translation-denial ]]; then
  echo "${LEANOS_SERIAL_19_BOOT} target=x86_64-q35 subjects=2 schedule=stale-translation-denial probe=cpl3-unmap-read contract=v1 controls=wp,smep,smap,pcid-off" > "$expected"
elif [[ "$scenario" == frame-budget ]]; then
  echo "${LEANOS_SERIAL_20_BOOT} target=x86_64-q35 subjects=2 schedule=frame-budget-v2 budgets=a:1,b:2 controls=wp,smep,smap" > "$expected"
elif [[ "$scenario" == capability-transfer ]]; then
  echo "${LEANOS_SERIAL_22_BOOT} target=x86_64-q35 subjects=2 schedule=capability-transfer-v1 controls=wp,smep,smap boundary=generated-composite" > "$expected"
elif [[ "$scenario" == inflight-revocation ]]; then
  echo "${LEANOS_SERIAL_23_BOOT} target=x86_64-q35 subjects=2 schedule=inflight-revocation-v1 controls=wp,smep,smap boundary=generated-composite" > "$expected"
elif (( fault_scenario )); then
  echo "${LEANOS_SERIAL_14_BOOT} target=x86_64-q35 subjects=2 schedule=fault-containment probe=${fault_probe} contract=v1 controls=wp,smep,smap" > "$expected"
elif [[ "$scenario" == direct-port-serial || "$scenario" == direct-port-debug ||
      "$scenario" == direct-port-in || "$scenario" == direct-port-pic ]]; then
  echo "${LEANOS_SERIAL_16_BOOT} target=x86_64-q35 subjects=2 schedule=direct-port-containment probe=${direct_port_probe_name} contract=v1 controls=wp,smep,smap" > "$expected"
elif [[ "$scenario" == divide-error || "$scenario" == breakpoint ]]; then
  echo "${LEANOS_SERIAL_18_BOOT} target=x86_64-q35 subjects=2 schedule=integer-fault-containment probe=${integer_fault_kind} contract=v1 controls=wp,smep,smap" > "$expected"
else
  echo "${LEANOS_SERIAL_10_BOOT} target=x86_64-q35 subjects=2 schedule=blocking-ipc controls=wp,smep,smap" > "$expected"
fi
echo "${LEANOS_SERIAL_15_DMA} snapshot=1 topology=0001000800020002 bus=0 scanned=256 present=5 optional-absent=1 writes=5 readbacks=5 initial-bus-masters=1 initial-bus-master-mask=16 bus-master=disabled readback=exact generated-result=0 stage=pre-cpl3 result=PASS" >> "$expected"
printf '%s\n' \
  ...
  "${LEANOS_SERIAL_19_TLB} mutable-leaf=checked address-space=2 page=7 states=boot,before,unmapped,after immutable-leaves=exact result=PASS" >> "$expected"
if [[ "$scenario" == frame-budget ]]; then
  ...
  echo "${LEANOS_SERIAL_20_FRAME} physical-frame=${frame_budget_physical} boot-published-frame=${frame_budget_boot_physical} prior-publications=0 distinct=1 source=scalar-stream-projection result=PASS" >> "$expected"
fi
awk -F '\t' "\$1 ~ /^[0-9]+\$/ { print \"${LEANOS_SERIAL_3_ORACLE} id=\" \$2 \" result=PASS\" }" "$corpus" >> "$expected"
echo "${LEANOS_SERIAL_17_ENTRY_MANIFEST} ordinary=8 extended=6,7 contained=0,3 auxiliary=1 terminal=2 extra=0 rsp0=entry-stack ist1=df-stack ist2=nmi-stack result=PASS" >> "$expected"
echo "${LEANOS_SERIAL_16_DIRECT_PORT_CONTROL} tr=40 limit=103 iomap=104 bitmap=absent iopl=0 stage=pre-cpl3 result=PASS" >> "$expected"
if [[ "$scenario" == fast-entry-syscall || "$scenario" == fast-entry-sysenter ]]; then
  echo "${LEANOS_SERIAL_14_FAST_ENTRY} cpu.vendor=AuthenticAMD mode=long64 syscall=1 sysenter=1 efer.sce=0 star=0 lstar=0 cstar=0 sfmask=0 sysenter.cs=0 sysenter.esp=0 sysenter.eip=0 writes=complete readback=exact result=PASS" >> "$expected"
  echo "${LEANOS_SERIAL_13_EXTENDED_STATE} cpuid.1.x87=1 cpuid.1.mmx=1 cpuid.1.sse=1 cpuid.1.sse2=1 cpuid.1.xsave=1 cpuid.1.osxsave=0 cpuid.1.avx=1 cpu=max result=PASS" >> "$expected"
elif [[ "$scenario" == extended-state || "$scenario" == extended-state-mmx ||
      "$scenario" == extended-state-sse || "$scenario" == extended-state-sse2 ||
      "$scenario" == extended-state-avx ]]; then
  echo "${LEANOS_SERIAL_13_EXTENDED_STATE} cpuid.1.x87=1 cpuid.1.mmx=1 cpuid.1.sse=1 cpuid.1.sse2=1 cpuid.1.xsave=1 cpuid.1.osxsave=0 cpuid.1.avx=1 cpu=max result=PASS" >> "$expected"
...
printf '%s\n' \
  ...
  "${LEANOS_SERIAL_6_CLEANUP} omitted=detected wrappers=checked entry=clac result=PASS" >> "$expected"
if [[ "$scenario" == fast-entry-syscall || "$scenario" == fast-entry-sysenter ]]; then
printf '%s\n' \
  "${LEANOS_SERIAL_14_FAST_ENTRY} event=enter subject=1 address-space=1 instruction=${extended_instruction} expected-vector=6" \
  "${LEANOS_SERIAL_14_FAST_ENTRY} event=deny subject=1 vector=6 instruction=${extended_instruction} alternate-target=unreached cleanup=complete peer=2" \
  ...
  "${LEANOS_SERIAL_14_FINAL} status=PASS denied=1 resumed-a=0 peer-ran=1 alternate-target=0" >> "$expected"
elif [[ "$scenario" == extended-state || "$scenario" == extended-state-mmx ||
      "$scenario" == extended-state-sse || "$scenario" == extended-state-sse2 ||
      "$scenario" == extended-state-avx ]]; then
printf '%s\n' \
  "${LEANOS_SERIAL_13_EXTENDED_STATE} event=enter subject=1 address-space=1 instruction=${extended_instruction} expected-vector=${extended_vector}" \
  "${LEANOS_SERIAL_13_EXTENDED_STATE} event=deny subject=1 vector=${extended_vector} instruction=${extended_instruction} bank-write=prevented cleanup=complete peer=2" \
  ...
  "${LEANOS_SERIAL_13_FINAL} status=PASS denied=1 resumed-a=0 peer-ran=1" >> "$expected"
elif (( fault_scenario )); then
printf '%s\n' \
  ...
  "${LEANOS_SERIAL_14_PF_WALK} page=${fault_page} expected-leaf=${fault_leaf} live-leaf=${fault_leaf} cause=${fault_cause} denial=${fault_cause} result=PASS" \
  "${LEANOS_SERIAL_14_FAULT_ENTRY} vector=14 error=${fault_error} access=${fault_access} protection=1 cr2=${fault_address} rip=${fault_rip_label} origin=cpl3 hardware=1 direct-call=0 subject=1 address-space=1 dispatch=0x00000000ff020202 cleanup=31 survivor=2${fault_canary} result=PASS" \
  ...
  "${LEANOS_SERIAL_14_FINAL} status=PASS faulting=terminated survivor=2 kernel-origin=fail-stop" >> "$expected"
elif [[ "$scenario" == direct-port-serial || "$scenario" == direct-port-debug ||
      "$scenario" == direct-port-in || "$scenario" == direct-port-pic ]]; then
printf '%s\n' \
  "${LEANOS_SERIAL_8_PAGING} root=A selected=1 resumed=1 result=PASS" \
  "${LEANOS_SERIAL_16_ENTER} subject=1 address-space=1 cpl=3 resources=owned" \
  "${LEANOS_SERIAL_16_DIRECT_PORT_DENIAL} subject=1 vector=13 error=0 origin=cpl3 port=${direct_port_port} direction=${direct_port_dir} width=byte purpose=user device-mutation=0 result=PASS" \
  ...
  "${LEANOS_SERIAL_8_PAGING} root=B selected=1 result=PASS" >> "$expected"
if [[ "$scenario" == direct-port-pic ]]; then
  echo "${LEANOS_SERIAL_16_DIRECT_PORT_CANARY} register=pic-mask port=33 programmed=255 observed=255 device-mutation=0 result=PASS" >> "$expected"
fi
printf '%s\n' \
  ...
  "${LEANOS_SERIAL_16_FINAL} status=PASS denied=1 resumed-a=0 peer-ran=1 device-mutation=0" >> "$expected"
elif [[ "$scenario" == divide-error || "$scenario" == breakpoint ]]; then
printf '%s\n' \
  "${LEANOS_SERIAL_8_PAGING} root=A selected=1 resumed=1 result=PASS" \
  "${LEANOS_SERIAL_18_ENTER} subject=1 address-space=1 cpl=3 resources=owned" \
  "$(leanos_serial 18 "${integer_fault_upper}-ENTRY") vector=${integer_fault_vector} error=none origin=cpl3 hardware=1 direct-call=0 saved-rip=${integer_fault_saved_rip} subject=1 address-space=1 result=PASS" \
  "$(leanos_serial 18 "${integer_fault_upper}-TERMINATE") subject=1 live=0 runnable=0 current=0 queued=0 resumable=0 resources=cap,memory,mapping,endpoint result=PASS" \
  "$(leanos_serial 18 "${integer_fault_upper}-DISPATCH") subject=2 address-space=2 source=lean-scheduler context=owned reason=${integer_fault_kind} result=PASS" \
  "${LEANOS_SERIAL_8_PAGING} root=B selected=1 result=PASS" \
  "$(leanos_serial 18 "${integer_fault_upper}-PEER") subject=2 address-space=2 stack=owned return=validated canaries=preserved resources=unchanged result=PASS" \
  "${LEANOS_SERIAL_18_FINAL} status=PASS faulting=terminated survivor=2 vector=${integer_fault_vector} reason=${integer_fault_kind} kernel-origin=fail-stop" >> "$expected"
elif [[ "$scenario" == capability-transfer ]]; then
printf '%s\n' \
  "${LEANOS_SERIAL_8_PAGING} root=A selected=1 resumed=1 result=PASS" \
  ...
  "${LEANOS_SERIAL_22_FINAL} status=PASS offer=1 sealed-denied=1 receipt=1 exact-handle=1 delegated-send=1 excess-right-denied=1 unrelated=unchanged" >> "$expected"
elif [[ "$scenario" == inflight-revocation ]]; then
printf '%s\n' \
  "${LEANOS_SERIAL_8_PAGING} root=A selected=1 resumed=1 result=PASS" \
  ...
  "${LEANOS_SERIAL_23_FINAL} status=PASS offer=1 revoke=1 envelope-and-pending=cleared canceled-receipt-denied=1 replacement-generation=8 canceled-handle-denied=1 fresh-send=1 unrelated=unchanged" >> "$expected"
elif [[ "$scenario" == frame-budget ]]; then
printf '%s\n' \
  "${LEANOS_SERIAL_8_PAGING} root=A selected=1 resumed=1 result=PASS" \
  "${LEANOS_SERIAL_20_ENTER} subject=1 address-space=1 cpl=3 budget=1 usage=0" \
  "${LEANOS_SERIAL_20_A_ALLOC} subject=1 address-space=1 budget=1 usage=1 object=10 handle=65536 physical-frame=${frame_budget_physical} user-page=4095 source=generated-mapping prior-publications=0 accepted=1" \
  ...
  "${LEANOS_SERIAL_20_SCRUB} physical-frame=${frame_budget_physical} bytes=4096 complete=1 before-publication=1" \
  "${LEANOS_SERIAL_20_B_PUBLISH} subject=2 object=21 handle=196609 generation=3 physical-frame=${frame_budget_physical} user-page=4095 source=generated-mapping fresh-lifetime=1" \
  ...
  "${LEANOS_SERIAL_20_FINAL} status=PASS a-exhausted=1 b-available=1 cleanup=1 scrub=1 fresh=1 stale-denied=1 ring3-reuse=1" >> "$expected"
elif [[ "$scenario" == preemption ]]; then
printf '%s\n' \
  "${LEANOS_SERIAL_6_COPY} direction=in length=4 cross-page=1 validated=1 user-df=1 kernel-df=cleared ac=cleared result=PASS" \
  "${LEANOS_SERIAL_6_COPY} direction=out length=4 cross-page=0 validated=1 user-df=1 kernel-df=cleared destination=verified-by-cpl3 ac=cleared result=PASS" \
  "${LEANOS_SERIAL_11_USER_FAULT} vector=14 error=5 origin=cpl3 address=zero contained=1 result=PASS" \
  "${LEANOS_SERIAL_5_ENTRY} subject=1 address-space=1 cpl=3 yielding=0" \
  ...
  "${LEANOS_SERIAL_8_PAGING} root=B selected=1 result=PASS" \
  "${LEANOS_SERIAL_5_SWITCH} subject=2 address-space=2 cr3=switched stack=initial contexts=separate" \
  "${LEANOS_SERIAL_5_SYSCALL} subject=2 caller=2 address-space=2 authorized=1 canaries=preserved" \
  ...
  "${LEANOS_SERIAL_5_FINAL} status=PASS ticks=2" >> "$expected"
elif [[ "$scenario" == stale-translation-denial ]]; then
printf '%s\n' \
  "${LEANOS_SERIAL_8_PAGING} root=B selected=1 result=PASS" \
  ...
  "${LEANOS_SERIAL_14_PF_SNAPSHOT} codec=1 width=19 words=1,14,4,28672,7,0,0,1,2,2,${stale_translation_cr3},15,${stale_translation_rip},35,582,${stale_translation_rsp},27,1,0 authorization=1 route=72057594037927937 result=PASS" \
  ...
  "${LEANOS_SERIAL_19_FINAL} status=PASS prefill=1 accepted-unmap=1 exact-invlpg=1 same-frame-reuse=1 scrub=complete new-owner-cpl3-read=1 replacement-canary=intact stale-access=page-fault old-observation=denied containment=1 incidental-cr3-reload=0" >> "$expected"
else
printf '%s\n' \
  "${LEANOS_SERIAL_8_PAGING} root=B selected=1 result=PASS" \
...
printf '%s\n' \
  "${LEANOS_SERIAL_11_ENTRY_ADVERSARIAL} attempted-vector=14 delivered=13 privileged-handler=unreached result=PASS" \
...
printf '%s\n' \
  ...
  "${LEANOS_SERIAL_10_FINAL} status=PASS blocks=1 wakes=1 deliveries=1" >> "$expected"
fi
fi
