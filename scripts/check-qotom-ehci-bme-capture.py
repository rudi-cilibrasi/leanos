...
D = runpy.run_path(str(Path(__file__).with_name('check-qotom-ehci-operational-capture.py')))
PREFIX = b'LEANOS-LAB/1 EHCI-BME '
...
def extract(raw, protocol):
    ...
    failure = protocol['FINAL'].encode() + b' status=FAIL reason=qotom-ehci-bme\n'
    if not indices:
    ...
    if previous is None or previous['status']:
        raise ValueError('BME without successful operational observation')
    match = re.fullmatch(PREFIX + rb'profile=qotom-bme-v1 index=10 status=' + DEC +
    ...
    if status < 9:
        if tuple(previous[k] for k in ('command','status_register','interrupt_enable','configured')) != (0x80000,0x1000,0,0):
            raise ValueError('BME write authority without stopped sample')
        prefix = protocol['PCI-HEADER'].encode() + b' codec=1 index=10 width=19 words='
        headers = [line for line in lines if line.startswith(prefix)]
        if len(headers) != 1 or int(headers[0][len(prefix):].split(b',')[4]) & 0xffff != 0x406:
    ...
    if status in (3,4,5,9,10,11):
        ...
        valid = attempted == 1 and before == 0x406
        if status == 0:
            valid = valid and after == 0x402
        elif status == 6:
    ...
    return projection, {'schema':'leanos-qotom-ehci-bme-observation-v1',
        'status':status,'write_attempted':attempted,'before_command':before,'after_command':after,
        'terminal_reason':'qotom-ehci-bme' if status else 'qotom-platform-pending',
