...
for module in PCIHeaderObservation QotomNativePCIFields; do
...
ld -r --gc-sections -u leanos_qotom_native_pci_header_check \
  "$build/PCIHeaderObservation.o" "$build/QotomNativePCIFields.o" -o "$build/native-pci.o"
...
if not functions <= allowed or 'leanos_qotom_native_pci_header_check' not in functions:
