...
D = runpy.run_path(str(Path(__file__).with_name('check-qotom-ahci-interrupt-capture.py')))
PREFIX = b'LEANOS-LAB/1 AHCI-BME '
...
def extract(raw, protocol):
    ...
    failure = protocol['FINAL'].encode() + b' status=FAIL reason=qotom-ahci-bme\n'
    if not indices:
    ...
    if previous is None or previous['status']:
        raise ValueError('BME without successful interrupt-disable observation')
    match = re.fullmatch(PREFIX + rb'profile=qotom-ahci-bme-v1 index=2 status=' + DEC +
    ...
    if status < 9:
        if tuple(previous[k] for k in ('write_attempted','before_control','after_control')) != (1,0x80000002,0x80000000):
            raise ValueError('BME write authority without accepted interrupt disable')
        ...
        if len(headers) != 1 or int(headers[0][len(prefix):].split(b',')[4]) & 0xffff != 0x7:
    ...
    if status in (3,4,5,9,10,11):
        ...
        valid = attempted == 1 and before == 0x7
        if status == 0:
            valid = valid and after == 0x3
        elif status == 6:
    ...
    return projection, {'schema':'leanos-qotom-ahci-bme-observation-v1',
        'status':status,'write_attempted':attempted,'before_command':before,'after_command':after,
        'terminal_reason':'qotom-ahci-bme' if status else 'qotom-platform-pending',
