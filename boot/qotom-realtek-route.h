...
static inline int qotom_realtek_route_valid_command(const struct pci_enumeration_header *endpoint,
        uint16_t endpoint_command,
    ...
    if (!qotom_realtek_header_valid_command(endpoint,endpoint_command) ||
        !qotom_rootport_header_valid(bridge) ||
...
static inline int qotom_realtek_route_valid(const struct pci_enumeration_header *endpoint,
        const struct pci_enumeration_header *bridge,
        enum qotom_rootport_bme_status status,const struct qotom_rootport_bme_result *prior) {
    return qotom_realtek_route_valid_command(endpoint,7,bridge,status,prior);
}
...
static inline uint32_t qotom_collect_realtek_routed_state_command(
        ...
        enum qotom_rootport_bme_status status,const struct qotom_rootport_bme_result *prior,
        uint16_t endpoint_command,struct qotom_realtek_state *out) {
    if (out) *out=(struct qotom_realtek_state){0};
    if (!config || !mmio || !out) return QOTOM_REALTEK_ARGUMENT;
    if (!caps || !qotom_realtek_route_valid_command(endpoint,endpoint_command,
            bridge,status,prior)) return 10;
    ...
    uint32_t result=qotom_collect_realtek_state_command(config,config_context,mmio,
        mmio_context,endpoint,endpoint_command,&sampled);
...
static inline uint32_t qotom_collect_realtek_routed_state(
    ...
    return qotom_collect_realtek_routed_state_command(config,config_context,mmio,mmio_context,
        endpoint,bridge,caps,status,prior,7,out);
}
#endif
