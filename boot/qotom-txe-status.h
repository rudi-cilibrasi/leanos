...
static inline int qotom_txe_header_valid_command(
        const struct pci_enumeration_header *initial,uint16_t expected_command) {
    return initial && !initial->bus && initial->device==26 && !initial->function &&
        initial->words[0]==UINT32_C(0x0f188086) &&
        initial->words[2]==UINT32_C(0x1080000e) &&
        !(initial->words[3]&UINT32_C(0x00ff0000)) &&
        (initial->words[1]&UINT32_C(0xffff))==expected_command;
}
...
static inline enum qotom_txe_status qotom_collect_txe_status_command(
    ...
    if(!qotom_txe_header_valid_command(initial,expected_command))
...
static inline enum qotom_txe_status qotom_collect_txe_status(
        ...
        const struct pci_enumeration_header *initial,
        struct qotom_txe_status_observation *out) {
    return qotom_collect_txe_status_command(read,context,initial,
        UINT16_C(0x0106),out);
}
#endif
