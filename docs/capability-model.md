...
reuses that handle. The userspace encoding admits only generations below its
reserved 48-bit maximum, so issuance must reject exhaustion before reuse. The
internal graph retains natural-number identities and therefore has no modeled
wraparound.
