...
| Interrupt descriptor/frame adapter | Loads the IDT/TSS, clears AC/DF, selects entry stacks, decodes x86 frame words, and bridges the generated manifest oracle; vector 2 uses a dedicated IST2 terminal assembly path | A descriptor, shape, bounds, latch, ABI, NMI-coalescing, or IST-switch mismatch can authorize the wrong handler/context or fail to reach the terminal latch | `LeanOS.InterruptEntry`, `scripts/check-entry-policy.sh`, `scripts/check-nmi-image-policy.sh`, runtime snapshots, and QEMU logs; monitor-injected vector 2 supplies tested delivery/IST2 evidence only after `NMI-READY`, while the inherited bootloader IDT, NMI before `privilege_init` completes, and hardware delivery/blocking remain trusted; the per-vector error-word and saved-RIP restart-class conventions for the modeled #DE (0), #BP (3), and #PF (14) classes are AMD64-manual assumptions consumed as normalized inputs, and the machine IDT/stub slice for vectors 0 and 3 is not yet installed by the boot image |
