...
| Single-core topology admission boundary | Discovers and copies the selected q35 ACPI MADT, decodes the bounded mixed entry stream, samples the BSP/executing APIC identities, and relies on non-BSP processors remaining dormant unless INIT/SIPI is issued | A forged, stale, incomplete, or misdecoded topology can publish the single-core premise on a multi-processor machine even when the Lean admission theorem is true | `LeanOS.BootTopology` and `SC-SINGLE-CORE-BOOT-ADMISSION`; bounded MADT entry framing/flag decoding, exact-length skipping for q35 I/O-APIC/interrupt-override/local-APIC-NMI records, fail-closed handling of other kinds, and admission are proved, while RSDP/MADT discovery, table-header/checksum validation, raw address translation, APIC-ID sampling, dormant-AP behavior, real INIT/SIPI delivery, generated code, and machine enforcement remain trusted until the hosted/boot boundary lands |
