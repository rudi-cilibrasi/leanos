...
| Interrupt descriptor/frame adapter | Loads the IDT/TSS, clears AC/DF, selects entry stacks, decodes x86 frame words, and bridges the generated manifest oracle; future vector-2 integration must also establish NMI delivery/blocking and the dedicated IST2 snapshot | A descriptor, shape, bounds, latch, ABI, NMI-coalescing, or IST-switch mismatch can authorize the wrong handler/context or fail to reach the terminal latch | `LeanOS.InterruptEntry`, `scripts/check-entry-policy.sh`, runtime snapshot, and current QEMU logs; vector 2 has hosted/final-ELF/QEMU generated-classifier evidence but no installed gate, IST2, or delivery evidence |
