...
- DMA behavior outside the finite Phase 2 q35 quarantine model and its explicit
  integrity-only device-control assumption; device-read confidentiality, device
  assignment, IOMMUs and DMA/interrupt-remapping refinement, devices other than
  the selected manifest/console/exit path, networking, storage services,
  interrupts, SMP, concurrency, userspace, or hostile physical access;
...
| Single-core topology admission boundary | Discovers and copies the selected q35 ACPI MADT, decodes the bounded mixed entry stream, samples the BSP/executing APIC identities, and relies on non-BSP processors remaining dormant unless INIT/SIPI is issued | A forged, stale, incomplete, or misdecoded topology can publish the single-core premise on a multi-processor machine even when the Lean admission theorem is true | `LeanOS.BootTopology` and `SC-SINGLE-CORE-BOOT-ADMISSION`; the bounded complete-table signature/length/checksum gate, fixed MADT-header removal, entry framing/flag decoding, exact-length skipping for q35 I/O-APIC/interrupt-override/local-APIC-NMI records, fail-closed handling of other kinds, and admission are proved, while RSDP/MADT discovery and unique selection, raw address translation/copy fidelity, APIC-ID sampling, dormant-AP behavior, real INIT/SIPI delivery, generated code, and machine enforcement remain trusted until the hosted/boot boundary lands |
| Interrupt descriptor/frame adapter | Publishes the bootstrap `boot_idt32`/`boot_idt64` tables from the reviewed entry prologue, loads the runtime IDT/TSS, clears AC/DF, selects entry stacks, decodes x86 frame words, and bridges the generated manifest oracle; vector 2 uses dedicated bootstrap terminal stubs before `privilege_init` and the dedicated IST2 terminal assembly path afterward | A descriptor, shape, bounds, latch, ABI, NMI-coalescing, or IST-switch mismatch can authorize the wrong handler/context or fail to reach the terminal latch | `LeanOS.InterruptEntry`, `LeanOS.BootInterruptPhase`, `scripts/check-entry-policy.sh`, `scripts/check-early-idt-policy.py`, `scripts/check-nmi-image-policy.sh`, runtime snapshots, and QEMU logs; monitor-injected vector 2 supplies tested delivery/IST2 evidence only after `NMI-READY`, while the reviewed eleven-instruction pre-`lidt` prologue, the one-instruction long-mode IDTR handoff, bootstrap-window NMI non-delivery, and hardware delivery/blocking remain trusted; the per-vector error-word and saved-RIP restart-class conventions for the modeled #DE (0), #BP (3), and #PF (14) classes are AMD64-manual assumptions consumed as normalized inputs, and the machine IDT/stub slice for vectors 0 and 3 is not yet installed by the boot image |
| Page-fault provenance adapter | Samples the vector-14 error word and CR2, supplies kernel-owned subject/address-space/CR3 and paging controls, invokes the exact generated decoder, and carries the accepted canonical snapshot to later policy | Stale CR2, reordered samples, incorrect U/S or access bits, stale controls, or an ABI/code-generation mismatch can attest the wrong page/access/context even when the Lean theorem is true | `LeanOS.InterruptEntry.normalizePageFault`, the 19-word version-one codec, generated `leanos_page_fault_demo`, hosted/boot oracle vectors, and negative proof fixtures; x86 delivery, temporal error-word/CR2 identity, assembly sampling, generated C, compiler/linker, QEMU, and hardware remain trusted, with no mapping/TLB runtime-state refinement claim |
| Composite generated dispatcher | Reconstructs one complete bounded `CompositeState`, decodes a normalized operation including generation-bound handle words, calls `FailStop.authoritativeGate`, and encodes its exact typed result and successor | A code-generation, scalar ABI, harness, compiler, or corpus-extraction defect can disagree with the proved codec/gate even when the Lean theorem is true | `LeanOS.CompositeDispatcher` proves canonical bounded reconstruction and direct one-step/finite-trace model refinement. Generated `CompositeDispatcher.c`, the C calling convention, hosted harness, compiler, SHA-256 manifest, and retained per-fixture diagnostics are trusted/tested evidence, not verified compilation or a final-binary refinement theorem |
...
| PCI snapshot and device-control boundary | Completely enumerates the selected q35 bus after firmware, clears and reads back every present function's PCI Command bus-master bit before CPL3, and gives that bit the memory-mutation effect assumed by the finite integrity-only quarantine model; no device-read confidentiality, IOMMU, DMA-remapping, or implementation-refinement claim is supplied | An omitted function, optional-absence/read-failure ambiguity, stale read-back, firmware/device disobedience, incorrect Command semantics, device reads outside the contract, or C/final-binary mismatch can violate a stronger property despite a valid model result | `LeanOS.DMAQuarantine`, `boot/kernel.c`, `docs/dma-quarantine.md`, construction TSV, and mandatory `LEANOS/15 DMA` QEMU records; tested boundary, not proved refinement |
