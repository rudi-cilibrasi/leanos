...
| SC-USER-RETURN-AUTHORITY | `user_return_authority_confinement` | `FailStop.accepted_user_return_has_bound_authority` | `BootPageTablePlan.compile`, `FailStop.selectReturnAuthority`, `FailStop.completeUserReturn` | Well-formed state and accepted outgoing return; selected subject/address-space view agrees with lifecycle ownership and exact compiled-plan root/user-text/user-stack leaves; `user_return_composite_entry_witness` establishes entry/syscall/return non-vacuity | Proved | Live installation of the compiled plan, generated C, assembly, compiler, QEMU, hardware |
| SC-USER-RETURN-FAILSTOP | `user_return_rejection_failstop` | `FailStop.rejected_user_return_composite_atomicity` | `FailStop.gate` | Running composite state with armed authority; authoritative normalized return request is rejected | Proved | Frame/context decoding before the modeled gate, generated C, assembly restore, terminal serial transport, compiler, QEMU, hardware |
