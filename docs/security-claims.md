...
| SC-USER-RETURN-AUTHORITY | `user_return_authority_confinement` | `FailStop.accepted_user_return_has_bound_authority` | `FailStop.selectReturnAuthority`, `FailStop.completeUserReturn` | Well-formed state and accepted outgoing return; selected subject/address-space view agrees with lifecycle ownership; `user_return_authority_reachable_witness` establishes non-vacuity | Proved | Installed page-table-view fidelity, generated C, assembly, compiler, QEMU, hardware |
