...
| SC-DMA-QUARANTINE | `dma_quarantine_preserves_complete_projection` | `DMAQuarantine.unowned_device_preserves_complete_projection` | `DMAQuarantine.validate`, `DMAQuarantine.DeviceContract`, and the finite q35 snapshot/control model | Accepted snapshot for topology version `0x0008_0002_0002`; named present unassigned function; explicit device contract that any device-originated mutation requires assignment and the PCI bus-master bit; `dma_quarantine_q35_nonvacuous` supplies the accepted nonempty witness | Proved | PCI enumeration/configuration semantics and completeness, firmware/QEMU/device obedience, hotplug, generated code, assembly/C/compiler, final-binary refinement, IOMMU and confidentiality/covert channels |
