#ifndef LEANOS_LAB_QOTOM_XHCI_OPERATIONAL_ARM_H
#define LEANOS_LAB_QOTOM_XHCI_OPERATIONAL_ARM_H
 ...
 * collector must refresh these samples and ownership before operational reads.
...
static inline int lab_xhci_operational_arm(struct lab_xhci_operational_window *window,
        ...
        enum qotom_xhci_smi_status status, const struct qotom_xhci_smi_result *prior) {
    if (!window) return 0;
    ...
    if (!caps || !prior || status != QOTOM_XHCI_SMI_OBSERVED ||
        prior->write_attempted != 1 || prior->before_control != 0x2000 || prior->after_control != 0) return 0;
