#ifndef LEANOS_LAB_QOTOM_HDA_BME_WINDOW_H
#define LEANOS_LAB_QOTOM_HDA_BME_WINDOW_H
#include "qotom-ecam-memory.h"

/* Only the captured HDA PCI Command word at ECAM 00:1b.0 + 4.
 * This type cannot grant general ECAM writes or change ring/stream state. */
struct lab_hda_bme_window {
...
static __attribute__((noreturn)) inline void lab_hda_bme_window_fault(
        struct lab_hda_bme_window *window) {
 ...
 */
static inline int lab_hda_bme_window_write(void *context, uint8_t bus,
        uint8_t device, uint8_t function, uint8_t offset, uint16_t value) {
    struct lab_hda_bme_window *window = context;
    if (!window) return 0;
    const unsigned armed = window->armed;
    window->armed = 0;
    const uint64_t mapped = UINT64_C(0x80000000e00d801b); /* RW, NX, supervisor, UC */
    struct lab_ecam_controls before, after;
    if (armed != 1 || bus || device != 27 || function || offset != 4 || value != 0x2 || !window->leaf ||
    ...
    *window->leaf = mapped;
    ...
    const int stored = window->store16(window->opaque, window->window + 4, 0x2);
    ...
    *window->leaf = saved;
    ...
    if (*window->leaf != saved || (observed & ~ad) != mapped ||
        ...
        lab_hda_bme_window_fault(window);
    return stored != 0;
}
#endif
