#ifndef LEANOS_LAB_QOTOM_XHCI_SMI_ARM_H
#define LEANOS_LAB_QOTOM_XHCI_SMI_ARM_H
 ...
 * admitted DWORD request. The SMI helper must refresh them before writing.
 * Every rejected rearm clears authority from a previously armed context. */
static inline int lab_xhci_smi_arm(struct lab_xhci_smi_window *window,
        ...
        enum qotom_xhci_handoff_status handoff_status, const struct qotom_xhci_handoff_result *handoff) {
    if (!window) return 0;
    window->armed = 0;
    window->leaf = 0;
    window->root = 0;
    window->window = 0;
    if (!handoff || handoff_status != QOTOM_XHCI_HANDOFF_OBSERVED ||
        handoff->write_attempted != 1 || !handoff->polls || handoff->polls > 100 ||
        handoff->last_support != 0x01000801 || handoff->final_control != 0x2000 ||
        handoff->verify_kind || handoff->verify_index || handoff->verify_expected || handoff->verify_observed) return 0;
