#ifndef LEANOS_LAB_QOTOM_EHCI_WINDOW_H
#define LEANOS_LAB_QOTOM_EHCI_WINDOW_H
#include "qotom-ecam-memory.h"

/* Separate address gate: this candidate admits only the three capability
 * register dwords of the observed EHCI BAR. It does not widen ECAM access. */
static inline int lab_ehci_read_leaf(uint64_t address, uint64_t *leaf) {
    if (!leaf || (address != UINT64_C(0xd0915000) &&
        address != UINT64_C(0xd0915004) && address != UINT64_C(0xd0915008))) return 0;
    *leaf = UINT64_C(0x80000000d0915019); /* supervisor, read-only, NX, PAT slot3 UC */
    return 1;
}

struct lab_ehci_window {
...
static __attribute__((noreturn)) inline void lab_ehci_window_fault(
        struct lab_ehci_window *window) {
...
static inline int lab_ehci_window_read(void *context, uint64_t address,
                                      uint32_t *value) {
    struct lab_ehci_window *window = context;
    uint64_t mapped;
    ...
    if (!window || !value || window->armed != 1 || !window->leaf ||
        ...
        (window->window & 4095u) || !lab_ehci_read_leaf(address, &mapped)) return 0;
