#ifndef LEANOS_LAB_QOTOM_EHCI_SMI_WINDOW_H
#define LEANOS_LAB_QOTOM_EHCI_SMI_WINDOW_H
#include "qotom-ecam-memory.h"

/* Only the captured EHCI legacy control/status dword at ECAM 00:1d.0 + 0x6c.
 * This type cannot grant general ECAM writes or change the BIOS semaphore. */
struct lab_ehci_smi_window {
...
static __attribute__((noreturn)) inline void lab_ehci_smi_window_fault(
        struct lab_ehci_smi_window *window) {
 ...
 */
static inline int lab_ehci_smi_window_write(void *context, uint8_t bus,
        uint8_t device, uint8_t function, uint8_t offset, uint32_t value) {
    struct lab_ehci_smi_window *window = context;
    if (!window) return 0;
    ...
    if (armed != 1 || bus || device != 29 || function || offset != 0x6c || value != 0 || !window->leaf ||
    ...
    *window->leaf = mapped;
    ...
    const int stored = window->store32(window->opaque, window->window + 0x6c, 0);
    ...
    *window->leaf = saved;
    ...
    if (*window->leaf != saved || (observed & ~ad) != mapped ||
        ...
        lab_ehci_smi_window_fault(window);
    return stored != 0;
}
#endif
