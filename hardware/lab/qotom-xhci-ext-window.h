#ifndef LEANOS_LAB_QOTOM_XHCI_EXT_WINDOW_H
#define LEANOS_LAB_QOTOM_XHCI_EXT_WINDOW_H
#include "qotom-ecam-memory.h"

/* Separate address gate: this candidate admits aligned extended-capability
 * reads in the upper half of the observed 64-KiB BAR. The bounded collector
 * validates each relative link; the seven-register capability gate is unchanged. */
static inline int lab_xhci_ext_read_leaf(uint64_t address, uint64_t *leaf) {
    if (!leaf || address < UINT64_C(0xd0908000) ||
        address > UINT64_C(0xd090fffc) || (address & 3)) return 0;
    *leaf = (address & ~UINT64_C(4095)) | UINT64_C(0x8000000000000019); /* supervisor, read-only, NX, PAT slot3 UC */
    return 1;
}

struct lab_xhci_ext_window {
...
static __attribute__((noreturn)) inline void lab_xhci_ext_window_fault(
        struct lab_xhci_ext_window *window) {
...
static inline int lab_xhci_ext_window_read(void *context, uint64_t address,
                                      uint32_t *value) {
    struct lab_xhci_ext_window *window = context;
    uint64_t mapped;
    ...
    if (!window || !value || window->armed != 1 || !window->leaf ||
        ...
        (window->window & 4095u) || !lab_xhci_ext_read_leaf(address, &mapped)) return 0;
    ...
    *window->leaf = saved;
    ...
    if (*window->leaf != saved || (observed & ~UINT64_C(0x20)) != mapped ||
        ...
        lab_xhci_ext_window_fault(window);
    if (!loaded) return 0;
    *value = sampled;
    return 1;
}
