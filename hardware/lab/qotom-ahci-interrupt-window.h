#ifndef LEANOS_LAB_QOTOM_AHCI_INTERRUPT_WINDOW_H
#define LEANOS_LAB_QOTOM_AHCI_INTERRUPT_WINDOW_H
#include "qotom-ecam-memory.h"

/* Only DWORD 80000000 at captured GHC (BAR + 4) is admitted.
 * This retains AE, clears IE, and leaves HR zero. No port write is admitted. */
struct lab_ahci_interrupt_window {
...
static __attribute__((noreturn)) inline void lab_ahci_interrupt_window_fault(
        struct lab_ahci_interrupt_window *window) {
 ...
 */
static inline int lab_ahci_interrupt_window_write(void *context, uint64_t address, uint32_t value) {
    struct lab_ahci_interrupt_window *window = context;
    if (!window) return 0;
    const unsigned armed = window->armed;
    window->armed = 0;
    const uint64_t mapped = UINT64_C(0x80000000d091601b); /* RW, NX, supervisor, UC */
    struct lab_ecam_controls before, after;
    if (armed != 1 || address != UINT64_C(0xd0916004) || value != UINT32_C(0x80000000) || !window->leaf ||
    ...
    *window->leaf = mapped;
    ...
    const int stored = window->store32(window->opaque, window->window + 4, UINT32_C(0x80000000));
    ...
    *window->leaf = saved;
    ...
    if (*window->leaf != saved || (observed & ~ad) != mapped ||
        ...
        lab_ahci_interrupt_window_fault(window);
    return stored != 0;
}
#endif
