#ifndef LEANOS_LAB_QOTOM_EHCI_BME_ARM_H
#define LEANOS_LAB_QOTOM_EHCI_BME_ARM_H
#include "qotom-ecam-firmware.h"
#include "qotom-ecam-root.h"
#include "qotom-ehci-bme-window.h"
#include "qotom-ehci-bme.h"
 ...
 * refreshes the binding through qotom_clear_ehci_bme before the write.
...
static inline int lab_ehci_bme_arm(struct lab_ehci_bme_window *w,
        ...
        enum qotom_ehci_operational_status status,const struct qotom_ehci_operational *prior) {
    if(!w)return 0;
    ...
    if(!header || header->bus || header->device!=29 || header->function ||
       ...
       (header->words[3]&UINT32_C(0x00ff0000)) || (header->words[1]&0xffff)!=0x406 ||
       header->words[4]!=QOTOM_EHCI_BAR || !caps ||
       caps->capbase!=UINT32_C(0x01000020) || caps->structural!=UINT32_C(0x00200008) ||
       caps->capability!=UINT32_C(0x00036881) || !smi ||
       smi_status!=QOTOM_EHCI_SMI_OBSERVED || smi->write_attempted!=1 ||
       (smi->before_control&~(QOTOM_EHCI_SMI_ENABLE|QOTOM_EHCI_SMI_STATUS)) ||
       (smi->after_control&~QOTOM_EHCI_SMI_STATUS) ||
       status!=QOTOM_EHCI_OPERATIONAL_OK || !qotom_ehci_stopped_sample(prior))return 0;
