#ifndef LEANOS_LAB_QOTOM_HDA_WINDOW_H
#define LEANOS_LAB_QOTOM_HDA_WINDOW_H
#include "qotom-ecam-memory.h"
#include "qotom-hda-observation.h"

/* Only the observed global register addresses and their documented widths. */
static inline int lab_hda_read_leaf(uint64_t address,uint8_t width,uint64_t *leaf) {
    uint64_t checked;
    if (!leaf || address<QOTOM_HDA_BAR || address>=QOTOM_HDA_BAR+16384 ||
        !qotom_hda_observation_address((uint32_t)(address-QOTOM_HDA_BAR),width,&checked) ||
        checked!=address) return 0;
    *leaf=UINT64_C(0x80000000d0910019); /* supervisor, RO, NX, PAT slot3 UC */
    return 1;
}

struct lab_hda_window {
...
static __attribute__((noreturn)) inline void lab_hda_window_fault(
        struct lab_hda_window *window) {
...
static inline int lab_hda_window_read(void *context, uint64_t address,
                                      uint8_t width, uint32_t *value) {
    struct lab_hda_window *window = context;
    uint64_t mapped;
    ...
    if (!window || !value || window->armed != 1 || !window->leaf ||
        ...
        (window->window & 4095u) || !lab_hda_read_leaf(address, width, &mapped)) return 0;
