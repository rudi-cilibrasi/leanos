#ifndef LEANOS_LAB_QOTOM_ECAM_WINDOW_H
#define LEANOS_LAB_QOTOM_ECAM_WINDOW_H
#include "qotom-ecam-memory.h"

struct lab_ecam_window {
...
static __attribute__((noreturn)) inline void lab_ecam_window_fault(
        struct lab_ecam_window *window) {
...
static inline int lab_ecam_window_read(void *context, uint64_t address,
                                      uint32_t *value) {
    struct lab_ecam_window *window = context;
    uint64_t mapped;
    ...
    if (!window || !value || window->armed != 1 || !window->leaf ||
        ...
        (window->window & 4095u) || !lab_ecam_read_leaf(address, &mapped)) return 0;
