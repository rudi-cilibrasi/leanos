#ifndef LEANOS_LAB_QOTOM_XHCI_SEMAPHORE_WINDOW_H
#define LEANOS_LAB_QOTOM_XHCI_SEMAPHORE_WINDOW_H
#include "qotom-ecam-memory.h"

/* Only the captured xHCI USBLEGSUP DWORD at BAR + 0x8460, with the
 * original BIOS-owned bit retained and OS-owned requested. No other address
 * or value is admitted, including legacy control/status at +0x8464. */
struct lab_xhci_semaphore_window {
...
static __attribute__((noreturn)) inline void lab_xhci_semaphore_window_fault(
        struct lab_xhci_semaphore_window *window) {
 ...
 */
static inline int lab_xhci_semaphore_window_write(void *context, uint64_t address, uint32_t value) {
    struct lab_xhci_semaphore_window *window = context;
    if (!window) return 0;
    ...
    if (armed != 1 || address != UINT64_C(0xd0908460) || value != UINT32_C(0x01010801) || !window->leaf ||
    ...
    *window->leaf = mapped;
    ...
    const int stored = window->store32(window->opaque, window->window + 0x460, UINT32_C(0x01010801));
    ...
    *window->leaf = saved;
    ...
    if (*window->leaf != saved || (observed & ~ad) != mapped ||
        ...
        lab_xhci_semaphore_window_fault(window);
    return stored != 0;
}
#endif
