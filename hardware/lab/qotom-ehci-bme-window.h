#ifndef LEANOS_LAB_QOTOM_EHCI_BME_WINDOW_H
#define LEANOS_LAB_QOTOM_EHCI_BME_WINDOW_H
#include "qotom-ecam-memory.h"

/* Only the captured EHCI PCI Command word at ECAM 00:1d.0 + 4.
 * This type cannot grant general ECAM writes or change the BIOS semaphore. */
struct lab_ehci_bme_window {
...
static __attribute__((noreturn)) inline void lab_ehci_bme_window_fault(
        struct lab_ehci_bme_window *window) {
 ...
 */
static inline int lab_ehci_bme_window_write(void *context, uint8_t bus,
        uint8_t device, uint8_t function, uint8_t offset, uint16_t value) {
    struct lab_ehci_bme_window *window = context;
    if (!window) return 0;
    ...
    if (armed != 1 || bus || device != 29 || function || offset != 4 || value != 0x402 || !window->leaf ||
    ...
    *window->leaf = mapped;
    ...
    const int stored = window->store16(window->opaque, window->window + 4, 0x402);
    ...
    *window->leaf = saved;
    ...
    if (*window->leaf != saved || (observed & ~ad) != mapped ||
        ...
        lab_ehci_bme_window_fault(window);
    return stored != 0;
}
#endif
