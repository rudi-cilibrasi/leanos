...
SSHPASS=freebsd python3 scripts/run-qotom-recovery-lab.py \
  ...
  --elf build/qotom-entry-integration-lab/leanos-qotom-lab.elf \
  --output build/qotom-entry-integration-lab/physical-entry-integration-20260912-retry8 \
  --cycles 1 --scenario watchdog-leanos --pci-diagnostic \
  --diagnostic-protocol build/qotom-entry-integration-lab/boot/serial-protocol.tsv \
  ...
  --copy-root-publication --entry-integration
