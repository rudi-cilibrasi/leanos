#ifndef LEANOS_LAB_QOTOM_AHCI_PORT_WINDOW_H
#define LEANOS_LAB_QOTOM_AHCI_PORT_WINDOW_H
#include "qotom-ecam-memory.h"

/* Separate address gate: this candidate admits only the six port
 * register dwords of the observed AHCI BAR. It does not widen ECAM access. */
static inline int lab_ahci_port_read_leaf(uint64_t address, uint64_t *leaf) {
    if (!leaf || (address != UINT64_C(0xd0916194) && address != UINT64_C(0xd0916198) &&
        address != UINT64_C(0xd09161a0) && address != UINT64_C(0xd09161a8) &&
        address != UINT64_C(0xd09161b4) && address != UINT64_C(0xd09161b8))) return 0;
    *leaf = UINT64_C(0x80000000d0916019); /* supervisor, read-only, NX, PAT slot3 UC */
    return 1;
}

struct lab_ahci_port_window {
...
static __attribute__((noreturn)) inline void lab_ahci_port_window_fault(
        struct lab_ahci_port_window *window) {
...
static inline int lab_ahci_port_window_read(void *context, uint64_t address,
                                      uint32_t *value) {
    struct lab_ahci_port_window *window = context;
    uint64_t mapped;
    ...
    if (!window || !value || window->armed != 1 || !window->leaf ||
        ...
        (window->window & 4095u) || !lab_ahci_port_read_leaf(address, &mapped)) return 0;
    ...
    *window->leaf = saved;
    ...
    if (*window->leaf != saved || (observed & ~UINT64_C(0x20)) != mapped ||
        ...
        lab_ahci_port_window_fault(window);
    if (!loaded) return 0;
    *value = sampled;
    return 1;
}
