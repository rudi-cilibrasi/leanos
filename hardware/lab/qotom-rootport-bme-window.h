#ifndef LEANOS_LAB_QOTOM_ROOTPORT_BME_WINDOW_H
#define LEANOS_LAB_QOTOM_ROOTPORT_BME_WINDOW_H
#include "qotom-ecam-memory.h"

/* Only the captured ROOTPORT PCI Command word at ECAM 00:1c.fn + 4.
 * This type cannot grant general ECAM writes or write routing or link control. */
struct lab_rootport_bme_window {
...
static __attribute__((noreturn)) inline void lab_rootport_bme_window_fault(
        struct lab_rootport_bme_window *window) {
 ...
 */
static inline int lab_rootport_bme_window_write(void *context, uint8_t bus,
        uint8_t device, uint8_t function, uint8_t offset, uint16_t value) {
    struct lab_rootport_bme_window *window = context;
    if (!window) return 0;
    const unsigned armed = window->armed;
    window->armed = 0;
    const uint64_t mapped = (UINT64_C(0x80000000e00e001b) + ((uint64_t)window->function << 12)); /* RW, NX, supervisor, UC */
    struct lab_ecam_controls before, after;
    if (armed != 1 || bus || device != 28 || window->function > 3 || function != window->function || offset != 4 || value != 0x3 || !window->leaf ||
    ...
    *window->leaf = saved;
    ...
    if (*window->leaf != saved || (observed & ~ad) != mapped ||
        ...
        lab_rootport_bme_window_fault(window);
    return stored != 0;
}
#endif
