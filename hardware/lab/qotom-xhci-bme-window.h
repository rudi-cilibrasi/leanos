#ifndef LEANOS_LAB_QOTOM_XHCI_BME_WINDOW_H
#define LEANOS_LAB_QOTOM_XHCI_BME_WINDOW_H
#include "qotom-ecam-memory.h"

/* Only the captured XHCI PCI Command word at ECAM 00:14.0 + 4.
 * This type cannot grant general ECAM writes or change the BIOS semaphore. */
struct lab_xhci_bme_window {
...
static __attribute__((noreturn)) inline void lab_xhci_bme_window_fault(
        struct lab_xhci_bme_window *window) {
 ...
 */
static inline int lab_xhci_bme_window_write(void *context, uint8_t bus,
        uint8_t device, uint8_t function, uint8_t offset, uint16_t value) {
    struct lab_xhci_bme_window *window = context;
    if (!window) return 0;
    const unsigned armed = window->armed;
    window->armed = 0;
    const uint64_t mapped = UINT64_C(0x80000000e00a001b); /* RW, NX, supervisor, UC */
    struct lab_ecam_controls before, after;
    if (armed != 1 || bus || device != 20 || function || offset != 4 || value != 0x2 || !window->leaf ||
    ...
    *window->leaf = saved;
    ...
    if (*window->leaf != saved || (observed & ~ad) != mapped ||
        ...
        lab_xhci_bme_window_fault(window);
    return stored != 0;
}
#endif
