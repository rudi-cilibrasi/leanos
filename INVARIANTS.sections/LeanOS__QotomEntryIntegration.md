...
This checkpoint accepts only the exact first synchronous Qotom CPL3 entry exercise: two entries, one completed return, distinct aligned subject and closed roots, active closed-root agreement, a validated hardware frame with user IF disabled, all fifteen saved general-purpose registers, entry readback and final return reload. It never grants general CPL3 authority.
