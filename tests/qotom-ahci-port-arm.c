...
int main(void) {
    ...
    struct lab_ahci_port_window w={.controls=controls,.invalidate=invalidate,.load32=load,.fault=fault};
#define ARM() lab_ahci_port_arm(&w,&v,lab_ecam_expected_tables,LAB_ECAM_FIRMWARE_TABLE_COUNT,0x200000,&h,QOTOM_AHCI_OK,&prior)
    ...
    assert(ARM());
    assert(!lab_ahci_port_arm(&w,&v,changed,LAB_ECAM_FIRMWARE_TABLE_COUNT,0x200000,&h,QOTOM_AHCI_OK,&prior));
    assert(!w.armed && !w.leaf && !w.root && !w.window);
    assert(ARM());w.load32=NULL;assert(!ARM() && !w.armed && !w.leaf);w.load32=load;
    ...
    for(unsigned i=0;i<5;++i)for(unsigned bit=0;bit<32;++bit) {
        ...
        if(i==1 && bit==1)assert(ARM());
        else assert(!ARM() && !w.armed && !w.leaf && !w.root && !w.window);
        *fields[i]^=1u<<bit;
    }
    for(unsigned status=1;status<=6;++status) {
        assert(ARM());assert(!lab_ahci_port_arm(&w,&v,lab_ecam_expected_tables,
