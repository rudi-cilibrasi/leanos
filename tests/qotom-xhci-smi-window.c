...
#include <setjmp.h>
#include <stdio.h>
#include "qotom-xhci-smi-window.h"
static struct lab_xhci_smi_window w;
...
static int store32(void *p,uint64_t a,uint32_t v) {
    assert(p==&w && a==0x400464 && v==0 && !w.armed);
...
static void reset(void) {
    ...
    w=(struct lab_xhci_smi_window){&leaf,0x150000,0x400000,1,&w,controls,invalidate,store32,fault};
}
static int request(void) {return lab_xhci_smi_window_write(&w,0xd0908464,0);}
int main(void) {
    ...
    for(uint64_t offset=0;offset<=0x10004;++offset) {
        if(offset==0x8464)continue;
        reset();assert(!lab_xhci_smi_window_write(&w,UINT64_C(0xd0900000)+offset,0));
        assert(!w.armed && !controls_count && !stores && !invalidations && leaf==saved);
    }
    for(unsigned bit=0;bit<64;++bit) {
        reset();assert(!lab_xhci_smi_window_write(&w,UINT64_C(0xd0908464)^(UINT64_C(1)<<bit),0));
        assert(!w.armed && !stores && !controls_count);
    }
    for(unsigned bit=0;bit<32;++bit) {
        reset();assert(!lab_xhci_smi_window_write(&w,0xd0908464,UINT32_C(0)^(UINT32_C(1)<<bit)));
        assert(!w.armed && !stores && !controls_count);
    }
    for(uint32_t low=0;low<65536;++low) {
        if(low==0)continue;
        reset();assert(!lab_xhci_smi_window_write(&w,0xd0908464,low));
        assert(!w.armed && !stores && !controls_count);
    }
    ...
    assert(!lab_xhci_smi_window_write(NULL,0xd0908464,0));
    puts("PASS xHCI smi window: exact MMIO DWORD request, consumed authority, ordered restore and terminal interference");
}
