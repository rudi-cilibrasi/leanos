...
int main(void) {
    ...
    struct lab_ehci_window w={.controls=controls,.invalidate=invalidate,.load32=load,.fault=fault};
#define ARM() lab_ehci_arm(&w,&v,lab_ecam_expected_tables,LAB_ECAM_FIRMWARE_TABLE_COUNT,0x200000,&h)
    ...
    puts("PASS EHCI arm: exact BAR, all 4096 aliases rejected, no device access, failed rearm revoked");
}
