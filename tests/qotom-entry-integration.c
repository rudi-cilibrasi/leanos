...
static uint64_t query(const uint64_t w[10],uint64_t word) {
    return leanos_qotom_entry_integration_query(
        w[0],w[1],w[2],w[3],w[4],w[5],w[6],w[7],w[8],w[9],word);
}

static int accepted(const uint64_t w[10]) {
    ++cases;
...
static int rejected(const uint64_t w[10]) {
    ++cases;
...
int main(void) {
    const uint64_t baseline[10]={2,1,UINT64_C(0x1a0000),
        UINT64_C(0x1a2000),UINT64_C(0x1a2000),1,0,15,1,1};
    if(!accepted(baseline))return 1;
    for(unsigned slot=0;slot<10;++slot) {
        uint64_t changed[10];
        for(unsigned i=0;i<10;++i)changed[i]=baseline[i];
        switch(slot) {
        ...
        case 5: case 8: case 9: changed[slot]=0;break;
        case 6: changed[slot]=1;break;
        case 7: changed[slot]=14;break;
        }
        if(!rejected(changed)) {
