...
int main(void) {
    ...
    struct lab_ahci_window w={.controls=controls,.invalidate=invalidate,.load32=load,.fault=fault};
#define ARM() lab_ahci_arm(&w,&v,lab_ecam_expected_tables,LAB_ECAM_FIRMWARE_TABLE_COUNT,0x200000,&h)
    ...
    assert(ARM());
    assert(!lab_ahci_arm(&w,&v,changed,LAB_ECAM_FIRMWARE_TABLE_COUNT,0x200000,&h));
