...
#include <setjmp.h>
#include <stdio.h>
#include "qotom-ahci-interrupt-window.h"
static struct lab_ahci_interrupt_window w;
...
static void invalidate(void *p,uint64_t a) {
    ...
    ++invalidations;
    if(invalidations==1)assert(leaf==UINT64_C(0x80000000d091601b) && !stores);
    else {
...
static int store32(void *p,uint64_t a,uint32_t v) {
    assert(p==&w && a==0x400004 && v==0x80000000 && !w.armed);
    assert(!stores && invalidations==1 && leaf==UINT64_C(0x80000000d091601b));
    ++stores;
...
static void reset(void) {
    ...
    w=(struct lab_ahci_interrupt_window){&leaf,0x150000,0x400000,1,&w,controls,invalidate,store32,fault};
}
static int request(void) {return lab_ahci_interrupt_window_write(&w,0xd0916004,0x80000000);}
int main(void) {
    ...
    for(uint64_t offset=0;offset<=0x10004;++offset) {
        ...
        reset();assert(!lab_ahci_interrupt_window_write(&w,UINT64_C(0xd0916000)+offset,0x80000000));
        assert(!w.armed && !controls_count && !stores && !invalidations && leaf==saved);
    }
    for(unsigned bit=0;bit<64;++bit) {
        reset();assert(!lab_ahci_interrupt_window_write(&w,UINT64_C(0xd0916004)^(UINT64_C(1)<<bit),0x80000000));
        assert(!w.armed && !stores && !controls_count);
    }
    for(unsigned bit=0;bit<32;++bit) {
        reset();assert(!lab_ahci_interrupt_window_write(&w,0xd0916004,UINT32_C(0x80000000)^(UINT32_C(1)<<bit)));
        assert(!w.armed && !stores && !controls_count);
    }
    for(uint32_t low=0;low<65536;++low) {
        if(low==0)continue;
        reset();assert(!lab_ahci_interrupt_window_write(&w,0xd0916004,UINT32_C(0x80000000)|low));
        assert(!w.armed && !stores && !controls_count);
    }
    ...
    assert(!lab_ahci_interrupt_window_write(NULL,0xd0916004,0));
    puts("PASS AHCI interrupt window: exact MMIO DWORD request, consumed authority, ordered restore and terminal interference");
}
