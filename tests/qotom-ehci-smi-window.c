...
static struct lab_ehci_smi_window w;
...
static int store32(void *p,uint64_t a,uint32_t v) {
    assert(p==&w && a==0x40006c && v==0 && !w.armed);
...
static void reset(void) {
    ...
    w=(struct lab_ehci_smi_window){&leaf,0x150000,0x400000,1,&w,controls,invalidate,store32,fault};
}
static int request(void) {return lab_ehci_smi_window_write(&w,0,29,0,0x6c,0);}
int main(void) {
    ...
    for(unsigned field=0;field<5;++field)for(unsigned value=0;value<256;++value) {
        unsigned valid[]={0,29,0,0x6c,0};
        if(value==valid[field])continue;
        reset();valid[field]=value;
        assert(!lab_ehci_smi_window_write(&w,valid[0],valid[1],valid[2],valid[3],valid[4]));
        assert(!w.armed && !controls_count && !stores && !invalidations && leaf==saved);
    }
    for(unsigned bit=8;bit<32;++bit) {
        reset();assert(!lab_ehci_smi_window_write(&w,0,29,0,0x6c,UINT32_C(1)<<bit));
    ...
    assert(!lab_ehci_smi_window_write(NULL,0,29,0,0x6c,0));
    puts("PASS EHCI SMI window: exact zero dword request, consumed authority, ordered restore and terminal interference");
}
