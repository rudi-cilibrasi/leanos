...
static struct lab_graphics_bme_window w;
...
static void invalidate(void *p,uint64_t a) {
    ...
    if(invalidations==1)assert(leaf==UINT64_C(0x80000000e001001b) && !stores);
    else {assert(invalidations==2 && leaf==saved && stores==1);if(restore_bad)leaf^=2;}
}
static int store16(void *p,uint64_t a,uint16_t v) {
    assert(p==&w && a==0x400004 && v==3 && !w.armed);
    assert(!stores && invalidations==1 && leaf==UINT64_C(0x80000000e001001b));
    ++stores;leaf|=0x60;if(leaf_bad)leaf^=4;return !store_failed;
}
...
static void reset(void) {
    ...
    w=(struct lab_graphics_bme_window){&leaf,0x150000,0x400000,1,&w,
        controls,invalidate,store16,fault};
}
static int request(void){return lab_graphics_bme_window_write(&w,0,2,0,4,3);}
int main(void) {
    reset();assert(request());assert(stores==1 && leaf==saved && controls_count==2);
    assert(!request() && stores==1 && invalidations==2);
    reset();store_failed=1;assert(!request());assert(stores==1 && leaf==saved);
    ...
    for(unsigned field=0;field<5;++field)for(unsigned value=0;value<256;++value) {
        unsigned valid[5]={0,2,0,4,3};if(value==valid[field])continue;
        reset();valid[field]=value;
        assert(!lab_graphics_bme_window_write(&w,valid[0],valid[1],valid[2],
            valid[3],(uint16_t)valid[4]));
        assert(!w.armed && !controls_count && !stores && !invalidations && leaf==saved);
    }
    for(unsigned value=0;value<65536;++value)if(value!=3) {
        reset();assert(!lab_graphics_bme_window_write(&w,0,2,0,4,(uint16_t)value));
    ...
    assert(!lab_graphics_bme_window_write(NULL,0,2,0,4,3));
    for(unsigned missing=0;missing<5;++missing) {
        ...
        assert(!request() && !w.armed && !controls_count && !stores && !invalidations);
    }
    ...
    puts("PASS graphics BME window: exact 0003 word request, consumed authority, restore and terminal interference");
}
