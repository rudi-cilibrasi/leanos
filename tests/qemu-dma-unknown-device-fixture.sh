...
case "${LEANOS_QEMU_FIXTURE_MODE:-success}" in
  ...
  wrong-reason) echo "${LEANOS_SERIAL_3_FINAL} status=FAIL reason=dma-identity" > "$log"; exit 35 ;;
  reached-cpl3) printf '%s\n%s\n' "$terminal" \
    "${LEANOS_SERIAL_5_ENTRY} subject=1 address-space=1 cpl=3 yielding=0" > "$log"; exit 35 ;;
  forged-pass) printf '%s\n%s\n' "$terminal" \
    "${LEANOS_SERIAL_15_DMA} snapshot=1 result=PASS" > "$log"; exit 35 ;;
  unrelated-guest-error) echo "${LEANOS_SERIAL_3_FINAL} status=FAIL reason=page-table" > "$log"; exit 39 ;;
  reset) : > "$log"; exit 0 ;;
  hang) sleep 10 ;;
  *) exit 2 ;;
esac
