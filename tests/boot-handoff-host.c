...
static lean_object *run_host(int argc, char **argv) {
    ...
    expect("accepted terminator", leanos_boot_handoff_fixture_query(0, 20), 0);

    expect("reversed status", leanos_boot_handoff_fixture_query(1, 1), 1);
    expect("reversed first entry kind", leanos_boot_handoff_fixture_query(1, 7), 2);
    expect("reversed second entry kind", leanos_boot_handoff_fixture_query(1, 10), 1);
    for (uint64_t word = 11; word < 20; ++word)
        expect("order-independent normalized projection",
               leanos_boot_handoff_fixture_query(1, word), expected[word]);
