...
#include "../build/qotom-quarantine-observation/cases.h"

extern void lean_initialize(void);
extern lean_object *initialize_leanos_LeanOS_QotomPCIQuarantineObservation(uint8_t);
extern void leanos_register_boundary_target(const char *, void *);

int main(void) {
    lean_initialize();
    lean_object *init = initialize_leanos_LeanOS_QotomPCIQuarantineObservation(1);
    ...
    leanos_register_boundary_target("leanos_qotom_pci_quarantine_observe",
        (void *)(uintptr_t)&leanos_qotom_pci_quarantine_observe);
    ...
    for (size_t i = 0; i < count; ++i) {
        ...
        uint64_t actual = leanos_qotom_pci_quarantine_observe(inventory_cases[i].count, words);
    ...
    printf("Hosted Qotom quarantine observation replay passed (%zu cases)\n", count);
    return 0;
}
