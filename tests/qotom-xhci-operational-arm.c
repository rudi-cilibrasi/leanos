...
int main(void) {
    ...
    struct lab_xhci_operational_window w={.controls=controls,.invalidate=invalidate,.load32=load,.fault=fault};
    enum qotom_xhci_smi_status status=QOTOM_XHCI_SMI_OBSERVED;
    struct qotom_xhci_smi_result prior={1,0x2000,0};
#define ARM() lab_xhci_operational_arm(&w,&v,lab_ecam_expected_tables,LAB_ECAM_FIRMWARE_TABLE_COUNT,0x200000,&h,&caps,status,&prior)
    ...
    assert(ARM());
    assert(!lab_xhci_operational_arm(&w,&v,lab_ecam_expected_tables,
    ...
    uint32_t *fields[]={&prior.write_attempted,&prior.before_control,&prior.after_control};
    for(unsigned f=0;f<3;++f)for(unsigned bit=0;bit<32;++bit) {
        assert(ARM());*fields[f]^=UINT32_C(1)<<bit;
        assert(!ARM() && !w.armed && !w.leaf && !w.root && !w.window);
        *fields[f]^=UINT32_C(1)<<bit;
    }
    for(unsigned st=1;st<=QOTOM_XHCI_SMI_READBACK;++st) {
        assert(ARM());status=(enum qotom_xhci_smi_status)st;
        assert(!ARM() && !w.armed);status=QOTOM_XHCI_SMI_OBSERVED;
    }
    assert(ARM());
    assert(!lab_xhci_operational_arm(&w,&v,lab_ecam_expected_tables,
        LAB_ECAM_FIRMWARE_TABLE_COUNT,0x200000,&h,&caps,status,NULL));
    assert(!w.armed && !w.leaf && !w.root && !w.window && !loads && !invalidations);
    puts("PASS XHCI operational arm: exact BAR, all 4096 aliases rejected, no device access, failed rearm revoked");
}
