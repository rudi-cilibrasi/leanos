...
int main(void) {
    ...
    struct qotom_ahci_interrupt_result interrupt={1,0x80000002,0x80000000};
    struct qotom_ahci_port stopped={6,0,0x50,0x123,0,0,6};
    struct lab_ahci_bme_window w={.controls=controls,.invalidate=invalidate,.store16=store,.fault=fault};
#define ARM() lab_ahci_bme_arm(&w,&v,lab_ecam_expected_tables,LAB_ECAM_FIRMWARE_TABLE_COUNT,0x200000,&h,QOTOM_AHCI_OK,&prior,QOTOM_AHCI_PORT_OK,&stopped,QOTOM_AHCI_INTERRUPTS_OK,&interrupt)
    ...
    const uint32_t masks[]={UINT32_MAX,0xffff,UINT32_MAX,0x00ff0000,UINT32_MAX};
    ...
    assert(ARM());
    assert(!lab_ahci_bme_arm(&w,&v,changed,LAB_ECAM_FIRMWARE_TABLE_COUNT,0x200000,&h,QOTOM_AHCI_OK,&prior,QOTOM_AHCI_PORT_OK,&stopped,QOTOM_AHCI_INTERRUPTS_OK,&interrupt));
    ...
    for(unsigned status=1;status<=6;++status) {
        assert(ARM());assert(!lab_ahci_bme_arm(&w,&v,lab_ecam_expected_tables,
            LAB_ECAM_FIRMWARE_TABLE_COUNT,0x200000,&h,status,&prior,QOTOM_AHCI_PORT_OK,&stopped,QOTOM_AHCI_INTERRUPTS_OK,&interrupt));
        assert(!w.armed && !w.leaf && !w.root && !w.window);
    }
    ...
    for(unsigned status=1;status<=9;++status) {
        assert(ARM());assert(!lab_ahci_bme_arm(&w,&v,lab_ecam_expected_tables,
            LAB_ECAM_FIRMWARE_TABLE_COUNT,0x200000,&h,QOTOM_AHCI_OK,&prior,status,&stopped,QOTOM_AHCI_INTERRUPTS_OK,&interrupt));
        assert(!w.armed && !w.leaf && !w.root && !w.window);
    }
    uint32_t *interrupt_fields[]={&interrupt.write_attempted,&interrupt.before_control,&interrupt.after_control};
    for(unsigned i=0;i<3;++i)for(unsigned bit=0;bit<32;++bit) {
        assert(ARM());*interrupt_fields[i]^=1u<<bit;
        assert(!ARM() && !w.armed && !w.leaf && !w.root && !w.window);
        *interrupt_fields[i]^=1u<<bit;
    }
    for(unsigned status=1;status<=11;++status) {
        assert(ARM());assert(!lab_ahci_bme_arm(&w,&v,lab_ecam_expected_tables,
            LAB_ECAM_FIRMWARE_TABLE_COUNT,0x200000,&h,QOTOM_AHCI_OK,&prior,QOTOM_AHCI_PORT_OK,&stopped,status,&interrupt));
        assert(!w.armed && !w.leaf && !w.root && !w.window);
    }
