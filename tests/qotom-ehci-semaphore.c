...
static struct lab_ehci_semaphore w;
...
static int store8(void *p,uint64_t a,uint8_t v) {
    assert(p==&w && a==0x40006b && v==1 && !w.armed);
...
static void reset(void) {
    ...
    w=(struct lab_ehci_semaphore){&leaf,0x150000,0x400000,1,&w,controls,invalidate,store8,fault};
}
static int request(void) {return lab_ehci_semaphore_write(&w,0,29,0,0x6b,1);}
int main(void) {
    ...
    for(unsigned field=0;field<5;++field)for(unsigned value=0;value<256;++value) {
        unsigned valid[]={0,29,0,0x6b,1};
        if(value==valid[field])continue;
        reset();valid[field]=value;
        assert(!lab_ehci_semaphore_write(&w,valid[0],valid[1],valid[2],valid[3],valid[4]));
    ...
    assert(!lab_ehci_semaphore_write(NULL,0,29,0,0x6b,1));
    puts("PASS EHCI semaphore window: exact byte request, consumed authority, ordered restore and terminal interference");
}
