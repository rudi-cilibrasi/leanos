...
#include <stdio.h>
#include <string.h>
#include "qotom-hda-window.h"

static struct lab_hda_window window;
static uint64_t leaf, saved, mapped;
static uint32_t output;
static uint64_t requested=0xd0910020;
...
int main(void) {
    ...
    reset();
    assert(lab_hda_window_read(&window, 0xd0910020, 4, &output));
    ...
    reset();
    assert(!lab_hda_window_read(0, 0xd0910020, 4, &output));
    assert(!lab_hda_window_read(&window, 0xd0910020, 4, 0));
    assert(!lab_hda_window_read(&window, 0xe0000100, 4, &output));
    assert(!control_calls && !invalidations && !loads);
    for(unsigned missing=0;missing<5;++missing) {
        ...
        assert(!lab_hda_window_read(&window,requested,requested_width,&output));
    ...
    reset(); post_bad = 1;
    if (!setjmp(terminal)) {
        (void)lab_hda_window_read(&window, 0xd0910020, 4, &output);
    ...
    for (unsigned offset=0;offset<16384;++offset)for(unsigned width=0;width<=8;++width) {
        ...
        int accepted=lab_hda_read_leaf(QOTOM_HDA_BAR+offset,width,&candidate);
        int valid=(offset==0 && width==2) || ((offset==2 || offset==3) && width==1) ||
            ((offset==8 || offset==32) && width==4);
        assert(accepted==valid);
        assert(candidate==(accepted?UINT64_C(0x80000000d0910019):42));
    }
    uint64_t candidate=42;
    assert(!lab_hda_read_leaf(UINT64_MAX,4,&candidate) && candidate==42);
    assert(!lab_hda_read_leaf(QOTOM_HDA_BAR-1,1,&candidate) && candidate==42);
    assert(!lab_hda_read_leaf(QOTOM_HDA_BAR+8,4,NULL));
    const unsigned offsets[]={0,2,3,8,32},widths[]={2,1,1,4,4};
    for(unsigned i=0;i<5;++i) {
        ...
        for(unsigned width=0;width<=8;++width)if(width!=requested_width) {
            reset();assert(!lab_hda_window_read(&window,requested,width,&output));
            assert(output==42 && leaf==saved && !loads && !invalidations && !control_calls);
        }
    }
    puts("Qotom HDA window: exact widths, ordered invalidation, private read, exact restore and terminal rejection PASS");
}
