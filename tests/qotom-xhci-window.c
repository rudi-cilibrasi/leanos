...
#include <stdio.h>
#include <string.h>
#include "qotom-xhci-window.h"

static struct lab_xhci_window window;
...
int main(void) {
    ...
    reset();
    assert(!lab_xhci_window_read(0, 0xd0900008, &output));
    assert(!lab_xhci_window_read(&window, 0xd0900008, 0));
    assert(!lab_xhci_window_read(&window, 0xe0000100, &output));
    assert(!control_calls && !invalidations && !loads);
    /* These paths must terminate after cleanup, never return a read result. */
    reset(); post_bad = 1;
    if (!setjmp(terminal)) {
        (void)lab_xhci_window_read(&window, 0xd0900008, &output);
    ...
    for (unsigned offset = 0; offset < 4096; ++offset) {
        ...
        int accepted = lab_xhci_read_leaf(UINT64_C(0xd0900000) + offset, &candidate);
        assert(accepted == (offset <= 24 && !(offset & 3)));
        assert(candidate == (accepted ? UINT64_C(0x80000000d0900019) : 42));
    }
    puts("Qotom XHCI window: ordered invalidation, private read, exact restore and terminal rejection PASS");
}
