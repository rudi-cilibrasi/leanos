...
int main(void) {
    ...
    struct lab_xhci_ext_window w={.controls=controls,.invalidate=invalidate,.load32=load,.fault=fault};
#define ARM() lab_xhci_ext_arm(&w,&v,lab_ecam_expected_tables,LAB_ECAM_FIRMWARE_TABLE_COUNT,0x200000,&h,&caps)
    ...
    assert(ARM());
    assert(!lab_xhci_ext_arm(&w,&v,lab_ecam_expected_tables,
        LAB_ECAM_FIRMWARE_TABLE_COUNT,0x200000,&h,NULL));
    assert(!w.armed && !w.leaf && !w.root && !w.window);
    assert(!loads && !invalidations);
    puts("PASS XHCI extended arm: exact BAR, all 4096 aliases rejected, no device access, failed rearm revoked");
}
