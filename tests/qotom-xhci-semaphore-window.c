...
static struct lab_xhci_semaphore_window w;
...
static void invalidate(void *p,uint64_t a) {
    ...
    ++invalidations;
    if(invalidations==1)assert(leaf==UINT64_C(0x80000000d090801b) && !stores);
    else {
        assert(invalidations==2 && leaf==saved && stores==1);
        if(restore_bad)leaf^=2;
    }
}
static int store32(void *p,uint64_t a,uint32_t v) {
    assert(p==&w && a==0x400460 && v==0x01010801 && !w.armed);
    assert(!stores && invalidations==1 && leaf==UINT64_C(0x80000000d090801b));
    ++stores;
...
static void reset(void) {
    ...
    w=(struct lab_xhci_semaphore_window){&leaf,0x150000,0x400000,1,&w,controls,invalidate,store32,fault};
}
static int request(void) {return lab_xhci_semaphore_window_write(&w,0xd0908460,0x01010801);}
int main(void) {
    ...
    for(uint64_t offset=0;offset<=0x10004;++offset) {
        if(offset==0x8460)continue;
        reset();assert(!lab_xhci_semaphore_window_write(&w,UINT64_C(0xd0900000)+offset,0x01010801));
        assert(!w.armed && !controls_count && !stores && !invalidations && leaf==saved);
    }
    for(unsigned bit=0;bit<64;++bit) {
        reset();assert(!lab_xhci_semaphore_window_write(&w,UINT64_C(0xd0908460)^(UINT64_C(1)<<bit),0x01010801));
        assert(!w.armed && !stores && !controls_count);
    }
    for(unsigned bit=0;bit<32;++bit) {
        reset();assert(!lab_xhci_semaphore_window_write(&w,0xd0908460,UINT32_C(0x01010801)^(UINT32_C(1)<<bit)));
        assert(!w.armed && !stores && !controls_count);
    }
    for(uint32_t low=0;low<65536;++low) {
        if(low==0x0801)continue;
        reset();assert(!lab_xhci_semaphore_window_write(&w,0xd0908460,0x01010000|low));
        assert(!w.armed && !stores && !controls_count);
    }
    for(unsigned which=0;which<4;++which) {
        reset();
        if(which==0)w.controls=NULL;
        if(which==1)w.invalidate=NULL;
        if(which==2)w.store32=NULL;
        if(which==3)w.fault=NULL;
        assert(!request() && !w.armed && !stores && !invalidations);
    }
    ...
    assert(!lab_xhci_semaphore_window_write(NULL,0xd0908460,0x01010801));
    puts("PASS xHCI semaphore window: exact MMIO DWORD request, consumed authority, ordered restore and terminal interference");
}
