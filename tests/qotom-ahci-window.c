...
static struct lab_ahci_window window;
static uint64_t leaf, saved, mapped;
static uint32_t output;
static uint64_t requested=0xd0916024;
...
int main(void) {
    ...
    reset();
    assert(!lab_ahci_window_read(0, 0xd0916024, &output));
    assert(!lab_ahci_window_read(&window, 0xd0916024, 0));
    assert(!lab_ahci_window_read(&window, 0xe0000100, &output));
    assert(!control_calls && !invalidations && !loads);
    /* These paths must terminate after cleanup, never return a read result. */
    reset(); post_bad = 1;
    if (!setjmp(terminal)) {
        (void)lab_ahci_window_read(&window, 0xd0916024, &output);
    ...
    for (unsigned offset = 0; offset < 4096; ++offset) {
        ...
        int accepted = lab_ahci_read_leaf(UINT64_C(0xd0916000) + offset, &candidate);
        assert(accepted == (offset == 0 || offset == 4 || offset == 12 || offset == 16 || offset == 36));
        assert(candidate == (accepted ? UINT64_C(0x80000000d0916019) : 42));
    }
    const unsigned offsets[]={0,4,12,16,36};
    for(unsigned i=0;i<5;++i) {
        requested=UINT64_C(0xd0916000)+offsets[i];reset();
        assert(lab_ahci_window_read(&window,requested,&output));
