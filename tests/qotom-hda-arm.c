...
int main(void) {
    ...
    struct lab_hda_window w={.controls=controls,.invalidate=invalidate,.load=load,.fault=fault};
#define ARM() lab_hda_arm(&w,&v,lab_ecam_expected_tables,LAB_ECAM_FIRMWARE_TABLE_COUNT,0x200000,&h)
    ...
    assert(ARM());
    assert(!lab_hda_arm(&w,&v,changed,LAB_ECAM_FIRMWARE_TABLE_COUNT,0x200000,&h));
    ...
    assert(ARM());
    assert(!lab_hda_arm(&w,NULL,lab_ecam_expected_tables,LAB_ECAM_FIRMWARE_TABLE_COUNT,0x200000,&h));
    assert(!w.armed && !w.leaf && !w.root && !w.window);
    assert(ARM());
    assert(!lab_hda_arm(&w,&v,lab_ecam_expected_tables,LAB_ECAM_FIRMWARE_TABLE_COUNT,0x200000,NULL));
    assert(!w.armed && !w.leaf && !w.root && !w.window);
    assert(!loads && !invalidations);
    puts("PASS HDA arm: exact BAR, all 4096 aliases to all four resource pages rejected, no device access, failed rearm revoked");
}
