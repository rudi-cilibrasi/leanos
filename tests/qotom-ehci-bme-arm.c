...
int main(void) {
    ...
    struct lab_ehci_bme_window w={.controls=controls,.invalidate=invalidate,.store16=load,.fault=fault};
    ...
    struct qotom_ehci_operational stopped={0x80000,0x1000,0,0};
    enum qotom_ehci_operational_status stopped_status=QOTOM_EHCI_OPERATIONAL_OK;
#define ARM() lab_ehci_bme_arm(&w,&v,lab_ecam_expected_tables,LAB_ECAM_FIRMWARE_TABLE_COUNT,0x200000,&h,&caps,status,&prior,stopped_status,&stopped)
    ...
    for(unsigned bit=0;bit<32;++bit) {
        ...
        assert(ARM());stopped.status^=mask;assert(!ARM() && !w.armed);stopped.status^=mask;
        assert(ARM());stopped.interrupt_enable=mask;assert(!ARM() && !w.armed);stopped.interrupt_enable=0;
        assert(ARM());stopped.configured=mask;assert(!ARM() && !w.armed);stopped.configured=0;
    }
    assert(ARM());stopped_status=QOTOM_EHCI_OPERATIONAL_FINAL;assert(!ARM() && !w.armed);stopped_status=QOTOM_EHCI_OPERATIONAL_OK;
    assert(ARM());h.words[1]=0x402;assert(!ARM() && !w.armed);h.words[1]=0x406;
    prior.after_control=QOTOM_EHCI_SMI_STATUS;assert(ARM());
    assert(!lab_ehci_bme_arm(&w,&v,lab_ecam_expected_tables,0,0x200000,&h,&caps,status,&prior,stopped_status,&stopped));
    assert(!w.armed && !w.leaf && !w.root && !w.window);
    assert(!loads && !invalidations);
    puts("PASS EHCI BME arm: exact BAR, all 4096 aliases rejected, no device access, failed rearm revoked");
}
