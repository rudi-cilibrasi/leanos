...
static struct lab_hda_bme_window w;
...
static void invalidate(void *p,uint64_t a) {
    ...
    ++invalidations;
    if(invalidations==1)assert(leaf==UINT64_C(0x80000000e00d801b) && !stores);
    else {
...
static int store16(void *p,uint64_t a,uint16_t v) {
    ...
    assert(!stores && invalidations==1 && leaf==UINT64_C(0x80000000e00d801b));
    ++stores;
...
static void reset(void) {
    ...
    w=(struct lab_hda_bme_window){&leaf,0x150000,0x400000,1,&w,controls,invalidate,store16,fault};
}
static int request(void) {return lab_hda_bme_window_write(&w,0,27,0,4,0x2);}
int main(void) {
    ...
    for(unsigned field=0;field<5;++field)for(unsigned value=0;value<256;++value) {
        unsigned valid[]={0,27,0,4,0x2};
        if(value==valid[field])continue;
        reset();valid[field]=value;
        assert(!lab_hda_bme_window_write(&w,valid[0],valid[1],valid[2],valid[3],valid[4]));
        assert(!w.armed && !controls_count && !stores && !invalidations && leaf==saved);
    }
    for(unsigned value=0;value<65536;++value) {
        if(value==0x2)continue;
        reset();assert(!lab_hda_bme_window_write(&w,0,27,0,4,(uint16_t)value));
    ...
    assert(!lab_hda_bme_window_write(NULL,0,27,0,4,0x2));
    ...
    puts("PASS HDA BME window: exact 0002 word request, consumed authority, ordered restore and terminal interference");
}
