...
set -e
sed -i \
  -e 's|LEANOS/10 BOOT target=x86_64-q35 subjects=2 schedule=blocking-ipc controls=wp,smep,smap|LEANOS/19 BOOT target=x86_64-q35 subjects=2 schedule=stale-translation-denial probe=cpl3-unmap-read contract=v1 controls=wp,smep,smap,pcid-off|' \
...
case "$mode" in
  ...
  reordered)
    sed -i \
      -e 's/^LEANOS\/19 TLB-CPL3 event=prefill /LEANOS\/19 TLB-CPL3 event=__SWAP__ /' \
      -e 's/^LEANOS\/19 TLB-CPL3 event=unmap /LEANOS\/19 TLB-CPL3 event=prefill /' \
      -e 's/^LEANOS\/19 TLB-CPL3 event=__SWAP__ /LEANOS\/19 TLB-CPL3 event=unmap /' \
      "$log"
    exit 33 ;;
  guest-error)
    sed -i 's/^LEANOS\/19 FINAL .*/LEANOS\/19 FINAL status=FAIL reason=stale-access/' "$log"
    exit 35 ;;
  reset) exit 0 ;;
