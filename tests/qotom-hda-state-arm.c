...
int main(void) {
    ...
    struct lab_hda_state_window w={.controls=controls,.invalidate=invalidate,.load=load,.fault=fault};
#define ARM() lab_hda_state_arm(&w,&v,lab_ecam_expected_tables,LAB_ECAM_FIRMWARE_TABLE_COUNT,0x200000,&h,QOTOM_HDA_OK,&prior)
    ...
    assert(ARM());
    assert(!lab_hda_state_arm(&w,&v,changed,LAB_ECAM_FIRMWARE_TABLE_COUNT,0x200000,&h,QOTOM_HDA_OK,&prior));
    ...
    assert(ARM());
    assert(!lab_hda_state_arm(&w,NULL,lab_ecam_expected_tables,LAB_ECAM_FIRMWARE_TABLE_COUNT,0x200000,&h,QOTOM_HDA_OK,&prior));
    assert(!w.armed && !w.leaf && !w.root && !w.window);
    assert(ARM());
    assert(!lab_hda_state_arm(&w,&v,lab_ecam_expected_tables,LAB_ECAM_FIRMWARE_TABLE_COUNT,0x200000,NULL,QOTOM_HDA_OK,&prior));
    assert(!w.armed && !w.leaf && !w.root && !w.window);
    ...
    for(unsigned status=1;status<=9;++status) {
        assert(ARM());assert(!lab_hda_state_arm(&w,&v,lab_ecam_expected_tables,
            LAB_ECAM_FIRMWARE_TABLE_COUNT,0x200000,&h,status,&prior));
        assert(!w.armed && !w.leaf && !w.root && !w.window);
    }
    assert(ARM());assert(!lab_hda_state_arm(&w,&v,lab_ecam_expected_tables,
        LAB_ECAM_FIRMWARE_TABLE_COUNT,0x200000,&h,QOTOM_HDA_OK,NULL));
    assert(!w.armed && !w.leaf && !w.root && !w.window);
    assert(!loads && !invalidations);
    puts("PASS HDA state arm: exact BAR, all 4096 aliases to all four resource pages rejected, no device access, failed rearm revoked");
}
