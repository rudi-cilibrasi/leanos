...
static struct lab_hda_state_window window;
static uint64_t leaf, saved, mapped;
static uint32_t output;
static uint64_t requested=0xd0910070;
...
static void reset(void) {
    ...
    assert(lab_hda_state_read_leaf(requested, requested_width, &mapped));
    output = 42;
    control_calls = invalidations = loads = faults = 0;
    pre_bad = post_bad = load_failed = leaf_interference = restore_interference = 0;
    window = (struct lab_hda_state_window){&leaf, 0x150000, 0x400000, 1,
...
int main(void) {
    ...
    reset();
    assert(lab_hda_state_window_read(&window, 0xd0910070, 4, &output));
    ...
    reset();
    assert(!lab_hda_state_window_read(0, 0xd0910070, 4, &output));
    assert(!lab_hda_state_window_read(&window, 0xd0910070, 4, 0));
    assert(!lab_hda_state_window_read(&window, 0xe0000100, 4, &output));
    assert(!control_calls && !invalidations && !loads);
    for(unsigned missing=0;missing<5;++missing) {
        ...
        assert(!lab_hda_state_window_read(&window,requested,requested_width,&output));
    ...
    reset(); post_bad = 1;
    if (!setjmp(terminal)) {
        (void)lab_hda_state_window_read(&window, 0xd0910070, 4, &output);
    ...
    for (unsigned offset=0;offset<16384;++offset)for(unsigned width=0;width<=8;++width) {
        uint64_t candidate=42;
        int accepted=lab_hda_state_read_leaf(QOTOM_HDA_BAR+offset,width,&candidate);
        int valid=((offset==0x4c || offset==0x5c) && width==1) || (offset==0x70 && width==4) ||
            (offset>=0x80 && offset<=0x160 && !(offset&31) && width==4);
        assert(accepted==valid);
        assert(candidate==(accepted?UINT64_C(0x80000000d0910019):42));
    }
    uint64_t candidate=42;
    assert(!lab_hda_state_read_leaf(UINT64_MAX,4,&candidate) && candidate==42);
    assert(!lab_hda_state_read_leaf(QOTOM_HDA_BAR-1,1,&candidate) && candidate==42);
    assert(!lab_hda_state_read_leaf(QOTOM_HDA_BAR+0x70,4,NULL));
    const unsigned offsets[]={0x4c,0x5c,0x70,0x80,0xa0,0xc0,0xe0,0x100,0x120,0x140,0x160},widths[]={1,1,4,4,4,4,4,4,4,4,4};
    for(unsigned i=0;i<11;++i) {
        requested=QOTOM_HDA_BAR+offsets[i];requested_width=widths[i];reset();
        assert(lab_hda_state_window_read(&window,requested,requested_width,&output));
        ...
        for(unsigned width=0;width<=8;++width)if(width!=requested_width) {
            reset();assert(!lab_hda_state_window_read(&window,requested,width,&output));
            assert(output==42 && leaf==saved && !loads && !invalidations && !control_calls);
        }
    }
    puts("Qotom HDA state window: exact widths, ordered invalidation, private read, exact restore and terminal rejection PASS");
}
