...
#include <stdio.h>
#include <string.h>
#include "qotom-xhci-operational-window.h"

static struct lab_xhci_operational_window window;
...
static int load32(void *opaque, uint64_t address, uint32_t *value) {
    assert(opaque == &window && address == 0x400084);
...
static void reset(void) {
    ...
    assert(lab_xhci_operational_read_leaf(0xd0900084, &mapped));
    output = 42;
    control_calls = invalidations = loads = faults = 0;
    pre_bad = post_bad = load_failed = leaf_interference = restore_interference = 0;
    window = (struct lab_xhci_operational_window){&leaf, 0x150000, 0x400000, 1,
                                     &window, controls, invalidate, load32, fault};
}

int main(void) {
    reset();
    assert(lab_xhci_operational_window_read(&window, 0xd0900084, &output));
    ...
    reset();
    assert(!lab_xhci_operational_window_read(0, 0xd0900084, &output));
    assert(!lab_xhci_operational_window_read(&window, 0xd0900084, 0));
    assert(!lab_xhci_operational_window_read(&window, 0xe0000100, &output));
    assert(!control_calls && !invalidations && !loads);
    /* These paths must terminate after cleanup, never return a read result. */
    reset(); post_bad = 1;
    if (!setjmp(terminal)) {
        (void)lab_xhci_operational_window_read(&window, 0xd0900084, &output);
    ...
    for (unsigned offset = 0; offset <= 0x10004; ++offset) {
        ...
        int accepted = lab_xhci_operational_read_leaf(UINT64_C(0xd0900000) + offset, &candidate);
        assert(accepted == (offset == 0x80 || offset == 0x84));
        assert(candidate == (accepted ? UINT64_C(0x80000000d0900019) : 42));
    }
    puts("Qotom XHCI operational window: ordered invalidation, private read, exact restore and terminal rejection PASS");
}
