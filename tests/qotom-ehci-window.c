...
#include <stdio.h>
#include <string.h>
#include "qotom-ehci-window.h"

static struct lab_ehci_window window;
...
static int load32(void *opaque, uint64_t address, uint32_t *value) {
    assert(opaque == &window && address == 0x400008);
...
int main(void) {
    ...
    reset();
    assert(!lab_ehci_window_read(0, 0xd0915008, &output));
    assert(!lab_ehci_window_read(&window, 0xd0915008, 0));
    assert(!lab_ehci_window_read(&window, 0xe0000100, &output));
    assert(!control_calls && !invalidations && !loads);
    /* These paths must terminate after cleanup, never return a read result. */
    reset(); post_bad = 1;
    if (!setjmp(terminal)) {
        (void)lab_ehci_window_read(&window, 0xd0915008, &output);
    ...
    for (unsigned offset = 0; offset < 4096; ++offset) {
        ...
        int accepted = lab_ehci_read_leaf(UINT64_C(0xd0915000) + offset, &candidate);
        assert(accepted == (offset == 0 || offset == 4 || offset == 8));
        assert(candidate == (accepted ? UINT64_C(0x80000000d0915019) : 42));
    }
    puts("Qotom EHCI window: ordered invalidation, private read, exact restore and terminal rejection PASS");
}
