...
case "${LEANOS_QEMU_FIXTURE_MODE:-success}" in
  ...
  reordered) echo "${LEANOS_SERIAL_11_ENTRY_HIGH_WATER} path=syscall observed-bytes=512 usable-bytes=16384 margin-bytes=15872 authority=diagnostic result=PASS" > "$log"; echo "$terminal" >> "$log"; exit 37 ;;
  ...
  forged-pass) printf '%s\n' "$terminal" "${LEANOS_SERIAL_11_ENTRY_STACK_OVERFLOW} status=PASS" > "$log"; exit 37 ;;
  guest-evidence) echo "${LEANOS_SERIAL_11_ENTRY_STACK_OVERFLOW} evidence=invalid status=FAIL" > "$log"; exit 39 ;;
  reset) : > "$log"; exit 0 ;;
