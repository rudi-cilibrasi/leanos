...
${LEANOS_SERIAL_8_PAGING} fixture=mutable-wrong-frame root=B level=pt page=7 expected=9223372036854792195 actual=9223372036854796291 result=REJECTED\\
${LEANOS_SERIAL_8_PAGING} fixture=mutable-publish-before-invalidation root=B level=pt page=7 expected=0 actual=9223372036854792195 result=REJECTED\\
${LEANOS_SERIAL_8_PAGING} fixture=mutable-unknown-state root=B level=pt page=7 expected=9223372036854804483 actual=9223372036854804483 result=REJECTED" "$log"
}
...
case "${LEANOS_QEMU_FIXTURE_MODE:-success}" in
  ...
  if [[ "$mode" == dma-missing ]]; then
    sed -i "/^$(leanos_serial_re 15 DMA) /d" "$log"
    ...
    sed -i "/^$(leanos_serial_re 15 DMA) snapshot=/s/generated-result=0/generated-result=1/" "$log"
    ...
    sed -i "/^$(leanos_serial_family_re 21) /d" "$log"
    ...
    sed -i "/^$(leanos_serial_re 21 VTD-ACTIVATE) /d" "$log"
...
frame-budget-global-counter|frame-budget-cross-charge|frame-budget-owner-forgery|frame-budget-relabel-success|frame-budget-partial-publication|frame-budget-double-credit|frame-budget-double-publication|frame-budget-register-leak|frame-budget-canary|frame-budget-stale-authorized|frame-budget-static-buffer|frame-budget-wrong-frame|frame-budget-non-ring3|frame-budget-missing|frame-budget-reordered|frame-budget-forged)
  ...
  case "$mode" in
    ...
    frame-budget-double-publication)
      sed -i \
        -e "/^$(leanos_serial_re 20 FRAME) /s/physical-frame=513/physical-frame=512/" \
        -e "/^$(leanos_serial_re 20 A-ALLOC) /s/physical-frame=513/physical-frame=512/" \
        -e "/^$(leanos_serial_re 20 SCRUB) /s/physical-frame=513/physical-frame=512/" \
        -e "/^$(leanos_serial_re 20 B-PUBLISH) /s/physical-frame=513/physical-frame=512/" \
        "$log"
      ;;
    ...
    frame-budget-missing) sed -i "/^$(leanos_serial_re 20 SCRUB) /d" "$log" ;;
    frame-budget-reordered)
      sed -i -e "s/^$(leanos_serial_re 20 SCRUB) /$(leanos_serial_family_re 20) __SWAP__ /" \
        -e "s/^$(leanos_serial_re 20 B-PUBLISH) /$(leanos_serial_re 20 SCRUB) /" \
        -e "s/^$(leanos_serial_family_re 20) __SWAP__ /$(leanos_serial_re 20 B-PUBLISH) /" "$log"
      ;;
    frame-budget-forged)
      sed -i "s|^${LEANOS_SERIAL_20_FINAL} .*|${LEANOS_SERIAL_20_FINAL} status=PASS a-exhausted=0 b-available=1 cleanup=1 scrub=1 fresh=1 stale-denied=1 ring3-reuse=1|" "$log"
      ;;
  esac
  exit 33
  ;;
...
if [[ "${LEANOS_QEMU_FIXTURE_MODE:-success}" == success &&
  ...
  set -e
  sed -i \
    -e "s|${LEANOS_SERIAL_10_BOOT} target=x86_64-q35 subjects=2 schedule=blocking-ipc controls=wp,smep,smap|${LEANOS_SERIAL_20_BOOT} target=x86_64-q35 subjects=2 schedule=frame-budget-v2 budgets=a:1,b:2 controls=wp,smep,smap|" \
    ...
    -e "/^$(leanos_serial_re 8 PAGING) root=B selected=1 result=PASS\$/d" "$log"
  sed -i \
    "/^$(leanos_serial_re 7 BOOTALLOC) status=PASS\$/a ${LEANOS_SERIAL_20_FRAME} physical-frame=513 boot-published-frame=512 prior-publications=0 distinct=1 source=scalar-stream-projection result=PASS" \
    "$log"
  cat >> "$log" <<EOF
${LEANOS_SERIAL_20_ENTER} subject=1 address-space=1 cpl=3 budget=1 usage=0
${LEANOS_SERIAL_20_A_ALLOC} subject=1 address-space=1 budget=1 usage=1 object=10 handle=65536 physical-frame=513 user-page=4095 source=generated-mapping prior-publications=0 accepted=1
${LEANOS_SERIAL_20_A_REJECT} subject=1 reason=budgetExhausted budget=1 usage=1 object=none capability=none mapping=none state=unchanged digest=0x4201
${LEANOS_SERIAL_20_DISPATCH} subject=2 address-space=2 source=generated-current result=PASS
${LEANOS_SERIAL_20_B_CONTEXT} subject=2 source=kernel-owned-fresh registers=15 canaries=fresh result=PASS
${LEANOS_SERIAL_20_B_ALLOC} subject=2 address-space=2 budget=2 usage=1 object=20 handle=131072 peer-a-usage=1 accepted=1
${LEANOS_SERIAL_20_CLEANUP} subject=1 operation=terminate objects=1 mappings=1 capacity-restored=1 repeated-credit=0 effect=flush invalidation=cr3 order=store,cr3,ack,publish checked=1
${LEANOS_SERIAL_20_SCRUB} physical-frame=513 bytes=4096 complete=1 before-publication=1
${LEANOS_SERIAL_20_B_PUBLISH} subject=2 object=21 handle=196609 generation=3 physical-frame=513 user-page=4095 source=generated-mapping fresh-lifetime=1
${LEANOS_SERIAL_20_STALE} handle=65536 old-subject=1 fresh-object=21 authorized=0 reason=stale-generation
${LEANOS_SERIAL_20_CANARY} subject=2 origin=cpl3 access=direct first=0 last=0 old=165 denied=1 result=PASS
${LEANOS_SERIAL_20_FINAL} status=PASS a-exhausted=1 b-available=1 cleanup=1 scrub=1 fresh=1 stale-denied=1 ring3-reuse=1
EOF
  exit "$status"
fi
if [[ "${LEANOS_QEMU_FIXTURE_MODE:-success}" == success &&
  ...
  sed -i \
    -e "s|${LEANOS_SERIAL_10_BOOT} target=x86_64-q35 subjects=2 schedule=blocking-ipc controls=wp,smep,smap|${LEANOS_SERIAL_14_BOOT} target=x86_64-q35 subjects=2 schedule=fault-containment probe=${fault_probe} contract=v1 controls=wp,smep,smap|" \
    -e "/^$(leanos_serial_family_re 9) /d" -e "/^$(leanos_serial_family_re 10) /d" \
    -e "/^$(leanos_serial_re 6 COPY) /d" -e "/^$(leanos_serial_re 11 USER-FAULT) /d" \
    -e "/^$(leanos_serial_re 11 ENTRY-HIGH-WATER) /d" \
...
EOF
  if [[ "$fault_scenario" == fault-nx-execute ]]; then
    ...
    printf "${LEANOS_SERIAL_14_PF_WALK} page=%s expected-leaf=%s live-leaf=%s cause=no-execute denial=no-execute result=PASS\\n" \
      "$fault_page" "$fault_leaf" "$fault_leaf" >> "$log"
    printf "${LEANOS_SERIAL_14_PF_SNAPSHOT} codec=1 width=19 words=1,14,21,%s,%s,2,1,1,1,1,%s,15,%s,35,534,%s,27,1,0 authorization=1 route=72057598316249602 result=PASS\\n" \
      "$fault_address" "$fault_page" \
      "$(fault_symbol_value page_map_level_4_a)" \
      "$fault_address" \
      "$(fault_symbol_value user_a_stack_top)" >> "$log"
    printf "${LEANOS_SERIAL_14_FAULT_ENTRY} vector=14 error=21 access=execute protection=1 cr2=%s rip=user-a-nx-fault-instruction origin=cpl3 hardware=1 direct-call=0 subject=1 address-space=1 dispatch=0x00000000ff020202 cleanup=31 survivor=2 payload-canary=armed result=PASS\\n" \
      "$fault_address" >> "$log"
    ...
    printf "${LEANOS_SERIAL_14_PF_WALK} page=%s expected-leaf=%s live-leaf=%s cause=not-writable denial=not-writable result=PASS\\n" \
      "$fault_page" "$fault_leaf" "$fault_leaf" >> "$log"
    printf "${LEANOS_SERIAL_14_PF_SNAPSHOT} codec=1 width=19 words=1,14,7,%s,%s,1,1,1,1,1,%s,15,%s,35,534,%s,27,1,0 authorization=1 route=72057598316249602 result=PASS\\n" \
      "$fault_address" "$fault_page" \
      "$(fault_symbol_value page_map_level_4_a)" \
      "$(fault_symbol_value user_a_write_fault_instruction)" \
      "$(fault_symbol_value user_a_stack_top)" >> "$log"
    printf "${LEANOS_SERIAL_14_FAULT_ENTRY} vector=14 error=7 access=write protection=1 cr2=%s rip=user-a-write-fault-instruction origin=cpl3 hardware=1 direct-call=0 subject=1 address-space=1 dispatch=0x00000000ff020202 cleanup=31 survivor=2 write-canary=unchanged result=PASS\\n" \
      "$fault_address" >> "$log"
  else
    printf '%s\n' \
      "${LEANOS_SERIAL_14_PF_WALK} page=0 expected-leaf=9223372036854775811 live-leaf=9223372036854775811 cause=supervisor denial=supervisor result=PASS" \
      >> "$log"
    printf "${LEANOS_SERIAL_14_PF_SNAPSHOT} codec=1 width=19 words=1,14,5,0,0,0,1,1,1,1,%s,15,%s,35,534,%s,27,1,0 authorization=1 route=72057598316249602 result=PASS\\n" \
      "$(fault_symbol_value page_map_level_4_a)" \
      "$(fault_symbol_value user_a_fault_instruction)" \
      "$(fault_symbol_value user_a_stack_top)" >> "$log"
    cat >> "$log" <<EOF
${LEANOS_SERIAL_14_FAULT_ENTRY} vector=14 error=5 access=read protection=1 cr2=0 rip=user-a-fault-instruction origin=cpl3 hardware=1 direct-call=0 subject=1 address-space=1 dispatch=0x00000000ff020202 cleanup=31 survivor=2 result=PASS
EOF
  fi
  cat >> "$log" <<EOF
${LEANOS_SERIAL_14_TERMINATE} subject=1 live=0 runnable=0 current=0 queued=0 resumable=0 resources=cap,memory,mapping,endpoint result=PASS
${LEANOS_SERIAL_14_DISPATCH} subject=2 address-space=2 source=lean-scheduler context=owned result=PASS
${LEANOS_SERIAL_8_PAGING} root=B selected=1 result=PASS
${LEANOS_SERIAL_14_PEER} subject=2 address-space=2 stack=owned return=validated canaries=preserved resources=unchanged result=PASS
${LEANOS_SERIAL_14_FINAL} status=PASS faulting=terminated survivor=2 kernel-origin=fail-stop
EOF
  exit "$status"
fi
case "${LEANOS_QEMU_FIXTURE_MODE:-success}" in
fault-direct-call|fault-wrong-error|fault-zero-error|fault-wrong-cr2|fault-wrong-rip|fault-wrong-access|fault-wrong-dispatch|fault-mapping-permission-drift|fault-snapshot-missing|fault-snapshot-duplicate|fault-snapshot-version|fault-snapshot-rip|fault-snapshot-authorization|fault-snapshot-route|fault-snapshot-reordered|fault-old-recovery|fault-stale-cr3|fault-cleanup-missing|fault-a-queued|fault-attacker-selection|fault-return-unvalidated|fault-peer-corrupt|fault-peer-cleaned|fault-forged-pass|fault-reordered|fault-kernel-relabeled|fault-global-fail|fault-nx-wrong-error|fault-nx-mapping-permission-drift|fault-nx-payload-forged)
  ...
  case "$mode" in
    ...
    fault-mapping-permission-drift)
      ...
      ;;
    fault-snapshot-missing) sed -i "/^$(leanos_serial_re 14 PF-SNAPSHOT) /d" "$log" ;;
    fault-snapshot-duplicate) sed -i "/^$(leanos_serial_re 14 PF-SNAPSHOT) /p" "$log" ;;
    ...
    fault-snapshot-reordered)
      snapshot="$(grep "^${LEANOS_SERIAL_14_PF_SNAPSHOT} " "$log")"
      sed -i "/^$(leanos_serial_re 14 PF-SNAPSHOT) /d" "$log"
      sed -i "/^$(leanos_serial_re 14 FAULT-ENTRY) /a $snapshot" "$log"
      ;;
    fault-old-recovery) sed -i "/^$(leanos_serial_re 14 TERMINATE) /d; /^$(leanos_serial_re 14 DISPATCH) /d" "$log" ;;
    ...
    fault-forged-pass) sed -i "/^$(leanos_serial_re 14 FAULT-ENTRY) /d; /^$(leanos_serial_re 14 TERMINATE) /d" "$log" ;;
    fault-reordered)
      sed -i -e "s/^$(leanos_serial_re 14 TERMINATE) /$(leanos_serial_family_re 14) __SWAP__ /" \
        -e "s/^$(leanos_serial_re 14 DISPATCH) /$(leanos_serial_re 14 TERMINATE) /" \
        -e "s/^$(leanos_serial_family_re 14) __SWAP__ /$(leanos_serial_re 14 DISPATCH) /" "$log"
      ;;
    fault-kernel-relabeled) sed -i 's/origin=cpl3/origin=kernel/' "$log" ;;
    fault-global-fail)
      sed -i "s/^$(leanos_serial_re 14 FINAL) .*/$(leanos_serial_re 14 FINAL) status=FAIL reason=kernel-fault/" "$log"
      exit 35
      ;;
...
${LEANOS_SERIAL_15_DMA_FUNCTION} manifest=1 topology=0001000800020002 bdf=0:31.3 present=1 vendor=32902 device=10544 class=787712 command-before=1 command-after=0 assigned=0 bridge=0 multifunction=1 policy=accepted" "$log"
  sed -i "/^$(leanos_serial_re 6 CONTROL)/i ${LEANOS_SERIAL_17_ENTRY_MANIFEST} ordinary=8 extended=6,7 contained=0,3 auxiliary=1 terminal=2 extra=0 rsp0=entry-stack ist1=df-stack ist2=nmi-stack result=PASS" "$log"
  sed -i "/^$(leanos_serial_re 6 CONTROL)/i ${LEANOS_SERIAL_16_DIRECT_PORT_CONTROL} tr=40 limit=103 iomap=104 bitmap=absent iopl=0 stage=pre-cpl3 result=PASS" "$log"
  sed -i \
    ...
    -e "/^$(leanos_serial_re 6 COPY) direction=out/a ${LEANOS_SERIAL_11_ENTRY_HIGH_WATER} path=user-page-fault observed-bytes=496 usable-bytes=16384 margin-bytes=15888 authority=diagnostic result=PASS\\n${LEANOS_SERIAL_11_USER_FAULT} vector=14 error=5 origin=cpl3 address=zero contained=1 result=PASS" \
    -e "s|${LEANOS_SERIAL_5_FINAL} status=PASS ticks=1|${LEANOS_SERIAL_5_TIMER} vector=32 source=pit mode=bounded-one-shot sequence=2 origin=cpl3 accepted=1\\n${LEANOS_SERIAL_5_CONTEXT} old-subject=2 old-address-space=2 new-subject=1 new-address-space=1 policy=round-robin\\n${LEANOS_SERIAL_8_PAGING} root=A selected=1 resumed=1 result=PASS\\n${LEANOS_SERIAL_5_SWITCH} subject=1 address-space=1 cr3=switched stack=resumed contexts=separate\\n${LEANOS_SERIAL_11_ENTRY_HIGH_WATER} path=timer-context-switch observed-bytes=512 usable-bytes=16384 margin-bytes=15872 authority=diagnostic result=PASS\\n${LEANOS_SERIAL_5_RESUME} subject=1 caller=1 address-space=1 frame=original canaries=preserved contexts=separate\\n${LEANOS_SERIAL_5_FINAL} status=PASS ticks=2|" \
    "$log"
  exit "$status"
fi
...
${LEANOS_SERIAL_15_DMA_FUNCTION} manifest=1 topology=0001000800020002 bdf=0:31.3 present=1 vendor=32902 device=10544 class=787712 command-before=1 command-after=0 assigned=0 bridge=0 multifunction=1 policy=accepted" "$log"
  ...
  sed -i "/^$(leanos_serial_re 6 CONTROL)/i ${LEANOS_SERIAL_16_DIRECT_PORT_CONTROL} tr=40 limit=103 iomap=104 bitmap=absent iopl=0 stage=pre-cpl3 result=PASS" "$log"
  sed -i \
    -e "s|${LEANOS_SERIAL_6_BOOT} target=x86_64-q35 subjects=2 schedule=one-shot-pit|${LEANOS_SERIAL_10_BOOT} target=x86_64-q35 subjects=2 schedule=blocking-ipc|" \
    -e "/^$(leanos_serial_family_re 5) /d" \
    -e "/^$(leanos_serial_re 8 PAGING) root=B selected=1 result=PASS\$/d" \
    -e "/^$(leanos_serial_re 6 COPY) direction=in/i ${LEANOS_SERIAL_8_PAGING} root=B selected=1 result=PASS\\n${LEANOS_SERIAL_10_IPC} event=enter subject=2 address-space=2 cpl=3 endpoint=10\\n${LEANOS_SERIAL_9_CAPREUSE} event=initial subject=2 handle=131072 endpoint=10 accepted=1\\n${LEANOS_SERIAL_9_CAPREUSE} event=clear slot=0 old-generation=2 result=PASS\\n${LEANOS_SERIAL_9_CAPREUSE} event=install slot=0 generation=3 endpoint=11 result=PASS\\n${LEANOS_SERIAL_9_CAPREUSE} event=stale-replay subject=2 handle=131072 rejected=1\\n${LEANOS_SERIAL_9_CAPREUSE} event=unchanged endpoint=11 mailbox=empty result=PASS\\n${LEANOS_SERIAL_9_CAPREUSE} event=fresh subject=2 handle=196608 endpoint=11 accepted=1\\n${LEANOS_SERIAL_9_CAPREUSE} status=PASS stale-effects=0 fresh-effects=1\\n${LEANOS_SERIAL_10_IPC} event=block subject=2 endpoint=10 empty=1 runnable=0 result=PASS\\n${LEANOS_SERIAL_8_PAGING} root=A selected=1 resumed=1 result=PASS\\n${LEANOS_SERIAL_10_IPC} event=dispatch subject=1 address-space=1 blocked-subject=2 trusted=1" \
    -e "/^$(leanos_serial_re 6 COPY) direction=out/a ${LEANOS_SERIAL_11_ENTRY_HIGH_WATER} path=user-page-fault observed-bytes=496 usable-bytes=16384 margin-bytes=15888 authority=diagnostic result=PASS\\n${LEANOS_SERIAL_11_USER_FAULT} vector=14 error=5 origin=cpl3 address=zero contained=1 result=PASS\\n${LEANOS_SERIAL_10_IPC} event=send sender=1 endpoint=10 payload0=1279607118 payload1=20307 accepted=1\\n${LEANOS_SERIAL_10_IPC} event=wake subject=2 ready-insertions=1 reserved=1 result=PASS\\n${LEANOS_SERIAL_8_PAGING} root=B selected=1 result=PASS\\n${LEANOS_SERIAL_10_IPC} event=dispatch subject=2 address-space=2 reservation=owned trusted=1\\n${LEANOS_SERIAL_10_IPC} event=deliver receiver=2 endpoint=10 sender=1 payload0=1279607118 payload1=20307 exact=1 canaries=preserved\\n${LEANOS_SERIAL_11_ENTRY_HIGH_WATER} path=syscall observed-bytes=512 usable-bytes=16384 margin-bytes=15872 authority=diagnostic result=PASS\\n${LEANOS_SERIAL_10_FINAL} status=PASS blocks=1 wakes=1 deliveries=1" \
    "$log"
  if [[ "${LEANOS_BOOT_SCENARIO:-blocking-ipc}" == entry-adversarial ]]; then
    sed -i "/^$(leanos_serial_re 6 COPY) direction=in/,/^$(leanos_serial_re 10 FINAL) status=PASS/d" "$log"
    sed -i "/event=dispatch subject=1/a ${LEANOS_SERIAL_11_ENTRY_ADVERSARIAL} attempted-vector=14 delivered=13 privileged-handler=unreached result=PASS\\n${LEANOS_SERIAL_11_ENTRY_ADVERSARIAL} attempted-vector=32 delivered=13 privileged-handler=unreached result=PASS\\n${LEANOS_SERIAL_16_DIRECT_PORT_DENIAL} subject=1 vector=13 error=0 origin=cpl3 port=244 direction=out width=byte purpose=user device-mutation=0 result=PASS\\n${LEANOS_SERIAL_16_DIRECT_PORT_TERMINATE} subject=1 live=0 runnable=0 current=0 queued=0 resumable=0 resources=cap,memory,mapping,endpoint result=PASS\\n${LEANOS_SERIAL_16_DIRECT_PORT_DISPATCH} subject=2 address-space=2 source=lean-scheduler context=owned result=PASS\\n${LEANOS_SERIAL_8_PAGING} root=B selected=1 result=PASS\\n${LEANOS_SERIAL_16_DIRECT_PORT_PEER} subject=2 address-space=2 stack=owned return=validated canaries=preserved resources=unchanged result=PASS\\n${LEANOS_SERIAL_16_FINAL} status=PASS denied=1 resumed-a=0 peer-ran=1 device-mutation=0" "$log"
  fi
  exit "$status"
fi
case "${LEANOS_QEMU_FIXTURE_MODE:-success}" in
entry-high-water-missing|entry-high-water-invalid|entry-high-water-duplicate|entry-high-water-reordered|entry-high-water-wrong-path)
  ...
  set -e
  case "$mode" in
    entry-high-water-missing) sed -i "/^$(leanos_serial_re 11 ENTRY-HIGH-WATER) /d" "$log" ;;
    entry-high-water-invalid) sed -i 's/margin-bytes=15872/margin-bytes=15871/' "$log" ;;
    entry-high-water-duplicate) sed -i "/^$(leanos_serial_re 11 ENTRY-HIGH-WATER) /p" "$log" ;;
...
omit-block|old-handoff|wrong-context|missing-wake|duplicate-wake|stolen-delivery|forged-pass)
  ...
  case "$mode" in
    ...
    forged-pass) sed -i "/^$(leanos_serial_re 10 IPC)/d" "$log" ;;
  esac
  exit 33
  ;;
esac
...
reuse-generation-ignored|reuse-truncated-handle|reuse-old-acts-replacement|reuse-forged-pass|reuse-wrong-caller|reuse-fresh-omitted|reuse-reordered)
  ...
  case "$mode" in
    ...
    reuse-forged-pass) sed -i "/^$(leanos_serial_re 9 CAPREUSE) event=/d" "$log" ;;
...
case "${LEANOS_QEMU_FIXTURE_MODE:-success}" in
legacy-success) printf '%s\n' "${LEANOS_SERIAL_6_BOOT} target=x86_64-q35 subjects=2 schedule=one-shot-pit controls=wp,smep,smap" "${LEANOS_SERIAL_15_DMA} snapshot=1 topology=0001000800020002 bus=0 scanned=256 present=5 optional-absent=1 writes=5 readbacks=5 bus-master=disabled readback=exact stage=pre-cpl3 result=PASS" > "$log"; printf '%s\n' "${LEANOS_SERIAL_8_PAGING} root=A selected=1 leaves=4096 policy=manifest result=PASS" "${LEANOS_SERIAL_8_PAGING} root=B selected=0 leaves=4096 policy=manifest result=PASS" "${LEANOS_SERIAL_8_PAGING} fixture=flip-present root=B level=pt page=0 expected=1 actual=0 result=REJECTED" "${LEANOS_SERIAL_8_PAGING} fixture=flip-user root=B level=pt page=1 expected=1 actual=5 result=REJECTED" "${LEANOS_SERIAL_8_PAGING} fixture=flip-writable root=B level=pt page=2 expected=1 actual=3 result=REJECTED" "${LEANOS_SERIAL_8_PAGING} fixture=flip-nx root=B level=pt page=3 expected=1 actual=9223372036854775809 result=REJECTED" "${LEANOS_SERIAL_8_PAGING} fixture=wrong-frame root=B level=pt page=0 expected=1 actual=4097 result=REJECTED" "${LEANOS_SERIAL_8_PAGING} fixture=ancestor-pointer root=B level=pml4 page=0 expected=8199 actual=12295 result=REJECTED" "${LEANOS_SERIAL_8_PAGING} fixture=ancestor-flags root=B level=pdpt page=0 expected=12295 actual=12291 result=REJECTED" "${LEANOS_SERIAL_8_PAGING} fixture=swapped-user-leaves root=B level=pt page=4 expected=16389 actual=20481 result=REJECTED" "${LEANOS_SERIAL_8_PAGING} fixture=extra-mapping root=B level=pt page=5 expected=0 actual=9223372036854796291 result=REJECTED" "${LEANOS_SERIAL_8_PAGING} fixture=entry-guard-mapping root=B level=pt page=6 expected=0 actual=9223372036854800387 result=REJECTED" "${LEANOS_SERIAL_8_PAGING} fixture=omitted-mapping root=B level=pt page=4 expected=16389 actual=0 result=REJECTED" "${LEANOS_SERIAL_8_PAGING} fixture=mutable-wrong-frame root=B level=pt page=7 expected=9223372036854792195 actual=9223372036854796291 result=REJECTED" "${LEANOS_SERIAL_8_PAGING} fixture=mutable-publish-before-invalidation root=B level=pt page=7 expected=0 actual=9223372036854792195 result=REJECTED" "${LEANOS_SERIAL_8_PAGING} fixture=mutable-unknown-state root=B level=pt page=7 expected=9223372036854804483 actual=9223372036854804483 result=REJECTED" "${LEANOS_SERIAL_8_PAGING} fixture=wrong-cr3 root=A level=cr3 page=0 expected=4096 actual=8192 result=REJECTED" "${LEANOS_SERIAL_7_HANDOFF} magic=valid info-bytes=1024 mmap-entries=8 result=PASS" "${LEANOS_SERIAL_7_MAP} boot-pages=4096 reported-top-mib=$((memory_mib - 1)) precedence=reserved result=PASS" "${LEANOS_SERIAL_7_ALLOC} frame=512 firmware-usable=1 boot-accessible=1 reserved=0 projection=exact-rich result=PASS" "${LEANOS_SERIAL_7_SCRUB} bytes=4096 zero=1 result=PASS" "${LEANOS_SERIAL_7_PUBLISH} object=1 owner=1 stale-object=denied result=PASS" "${LEANOS_SERIAL_7_BOOTALLOC} status=PASS" >> "$log"; awk -F '\t' "\$1 ~ /^[0-9]+\$/ { print \"${LEANOS_SERIAL_3_ORACLE} id=\" \$2 \" result=PASS\" }" "$LEANOS_ORACLE_CORPUS" >> "$log"; printf '%s\n' "${LEANOS_SERIAL_6_CONTROL} cr0.wp=1 cr0.em=1 cr0.mp=1 cr0.ts=1 cr4.osfxsr=0 cr4.osxmmexcpt=0 cr4.osxsave=0 cr4.pke=0 cr4.smep=1 cr4.smap=1 ac=0 stage=exception-path-ready" "${LEANOS_SERIAL_4_PROBE} kind=wp vector=14 error=3 origin=kernel address=kernel-text policy=fatal result=PASS" "${LEANOS_SERIAL_4_PROBE} kind=smep vector=14 error=17 origin=kernel address=user-a-text policy=fatal result=PASS" "${LEANOS_SERIAL_6_PROBE} kind=smap-direct vector=14 origin=kernel ac=0 result=PASS" "${LEANOS_SERIAL_6_POLICY} zero=accept max=accept unmapped=reject readonly=reject overflow=reject noncanonical=reject wrong-subject=reject stale=reject atomic=PASS" "${LEANOS_SERIAL_6_CLEANUP} omitted=detected wrappers=checked entry=clac result=PASS" "${LEANOS_SERIAL_6_COPY} direction=in length=4 cross-page=1 validated=1 user-df=1 kernel-df=cleared ac=cleared result=PASS" "${LEANOS_SERIAL_6_COPY} direction=out length=4 cross-page=0 validated=1 user-df=1 kernel-df=cleared destination=verified-by-cpl3 ac=cleared result=PASS" "${LEANOS_SERIAL_5_ENTRY} subject=1 address-space=1 cpl=3 yielding=0" "${LEANOS_SERIAL_5_TIMER} vector=32 source=pit mode=one-shot origin=cpl3 accepted=1" "${LEANOS_SERIAL_5_CONTEXT} old-subject=1 old-address-space=1 new-subject=2 new-address-space=2 policy=round-robin" "${LEANOS_SERIAL_8_PAGING} root=B selected=1 result=PASS" "${LEANOS_SERIAL_5_SWITCH} subject=2 address-space=2 cr3=switched stack=restored ticks-masked=1" "${LEANOS_SERIAL_5_SYSCALL} subject=2 caller=2 address-space=2 authorized=1 canaries=preserved" "${LEANOS_SERIAL_5_FINAL} status=PASS ticks=1" >> "$log"; exit 33;;
missing-paging) set +e; LEANOS_QEMU_FIXTURE_MODE=success "$0" "$@"; set -e; sed -i "/$(leanos_serial_re 8 PAGING)/d" "$log"; exit 33;;
missing-scrub) set +e; LEANOS_QEMU_FIXTURE_MODE=success "$0" "$@"; set -e; sed -i "/$(leanos_serial_re 7 SCRUB)/d" "$log"; exit 33;;
...
reordered-allocation) set +e; LEANOS_QEMU_FIXTURE_MODE=success "$0" "$@"; set -e; sed -i "s|${LEANOS_SERIAL_7_SCRUB} bytes=4096 zero=1 result=PASS|__SCRUB__|; s|${LEANOS_SERIAL_7_PUBLISH} object=1 owner=1 stale-object=denied result=PASS|${LEANOS_SERIAL_7_SCRUB} bytes=4096 zero=1 result=PASS|; s|__SCRUB__|${LEANOS_SERIAL_7_PUBLISH} object=1 owner=1 stale-object=denied result=PASS|" "$log"; exit 33;;
interrupts-disabled|timer-missing) set +e; LEANOS_QEMU_FIXTURE_MODE=success "$0" "$@"; set -e; sed -i "/$(leanos_serial_re 5 TIMER)/d" "$log"; exit 33;;
...
duplicate-tick) set +e; LEANOS_QEMU_FIXTURE_MODE=success "$0" "$@"; set -e; sed -i "/$(leanos_serial_re 5 TIMER)/p" "$log"; exit 33;;
