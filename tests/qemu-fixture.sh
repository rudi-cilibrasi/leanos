...
success) echo 'LEANOS/2 BOOT target=x86_64-q35 entry=int80' > "$log"; awk -F '\t' '$1 ~ /^[0-9]+$/ { print "LEANOS/2 ORACLE id=" $2 " result=PASS" }' "$LEANOS_ORACLE_CORPUS" >> "$log"; printf '%s\n' 'LEANOS/2 USER cpl=3' 'LEANOS/2 SYSCALL kind=authorized result=accepted' 'LEANOS/2 SYSCALL kind=forged result=rejected' 'LEANOS/2 FAULT vector=14 class=user-supervisor-access contained=1' 'LEANOS/2 RESUME kernel=1' 'LEANOS/2 FINAL status=PASS' >> "$log"; exit 33;;
