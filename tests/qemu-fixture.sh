...
success) echo 'LEANOS/3 BOOT target=x86_64-q35 subjects=2 schedule=fixed' > "$log"; awk -F '\t' '$1 ~ /^[0-9]+$/ { print "LEANOS/3 ORACLE id=" $2 " result=PASS" }' "$LEANOS_ORACLE_CORPUS" >> "$log"; tail -n +4 scripts/run-image.sh >/dev/null; printf '%s\n' 'LEANOS/3 SUBJECT id=1 address-space=1 cpl=3' 'LEANOS/3 IPC op=receive subject=1 result=denied reason=missing-receive' 'LEANOS/3 IPC op=send subject=1 result=accepted payload=4c45414e:4f53 supplied-sender=99' 'LEANOS/3 HANDOFF from=1 to=2 address-space=2 cr3=switched' 'LEANOS/3 SUBJECT id=2 address-space=2 cpl=3' 'LEANOS/3 IPC op=send subject=2 result=denied reason=missing-send' 'LEANOS/3 IPC op=receive subject=2 result=delivered sender=1 payload=4c45414e:4f53' 'LEANOS/3 IPC supplied-sender=99 trusted=0 capability-transfer=none' 'LEANOS/3 FAULT subject=2 vector=14 class=user-supervisor-access contained=1' 'LEANOS/3 RESUME kernel=1' 'LEANOS/3 FINAL status=PASS' >> "$log"; exit 33;;
