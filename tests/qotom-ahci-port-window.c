...
static struct lab_ahci_port_window window;
static uint64_t leaf, saved, mapped;
static uint32_t output;
static uint64_t requested=0xd0916198;
...
static int load32(void *opaque, uint64_t address, uint32_t *value) {
    assert(opaque == &window && address == 0x400000+(requested&4095));
...
static void reset(void) {
    ...
    assert(lab_ahci_port_read_leaf(requested, &mapped));
    output = 42;
    control_calls = invalidations = loads = faults = 0;
    pre_bad = post_bad = load_failed = leaf_interference = restore_interference = 0;
    window = (struct lab_ahci_port_window){&leaf, 0x150000, 0x400000, 1,
                                     &window, controls, invalidate, load32, fault};
}

int main(void) {
    reset();
    assert(lab_ahci_port_window_read(&window, 0xd0916198, &output));
    ...
    reset();
    assert(!lab_ahci_port_window_read(0, 0xd0916198, &output));
    assert(!lab_ahci_port_window_read(&window, 0xd0916198, 0));
    assert(!lab_ahci_port_window_read(&window, 0xe0000100, &output));
    assert(!control_calls && !invalidations && !loads);
    /* These paths must terminate after cleanup, never return a read result. */
    reset(); post_bad = 1;
    if (!setjmp(terminal)) {
        (void)lab_ahci_port_window_read(&window, 0xd0916198, &output);
    ...
    for (unsigned offset = 0; offset < 4096; ++offset) {
        ...
        int accepted = lab_ahci_port_read_leaf(UINT64_C(0xd0916000) + offset, &candidate);
        assert(accepted == (offset == 0x194 || offset == 0x198 || offset == 0x1a0 || offset == 0x1a8 || offset == 0x1b4 || offset == 0x1b8));
        assert(candidate == (accepted ? UINT64_C(0x80000000d0916019) : 42));
    }
    const unsigned offsets[]={0x194,0x198,0x1a0,0x1a8,0x1b4,0x1b8};
    for(unsigned i=0;i<6;++i) {
        requested=UINT64_C(0xd0916000)+offsets[i];reset();
        assert(lab_ahci_port_window_read(&window,requested,&output));
        assert(output==0x12345678 && leaf==saved && loads==1 && invalidations==2);
    }
    puts("Qotom AHCI window: ordered invalidation, private read, exact restore and terminal rejection PASS");
}
