...
def errorMask (entries returns incomingRoot closedRoot activeRoot frame gprs
  ...
  (if gprs == 15 then 0 else 64) +
  (if closeReadback == 1 then 0 else 128) +
  (if returnReload == 1 then 0 else 256)

def accepted (entries returns incomingRoot closedRoot activeRoot frame gprs
    closeReadback returnReload : UInt64) : Bool :=
  errorMask entries returns incomingRoot closedRoot activeRoot frame gprs
...
def query (entries returns incomingRoot closedRoot activeRoot frame gprs
    closeReadback returnReload word : UInt64) : UInt64 :=
  let ok := accepted entries returns incomingRoot closedRoot activeRoot frame
  ...
  if word == 0 then 1
    ...
    activeRoot frame gprs closeReadback returnReload
...
theorem query_never_authorizes_cpl3 entries returns incomingRoot closedRoot
    activeRoot frame gprs closeReadback returnReload :
    query entries returns incomingRoot closedRoot activeRoot frame gprs
      closeReadback returnReload 6 = 0 := by
  simp [query]

theorem checkpoint_claim_requires_acceptance entries returns incomingRoot
    closedRoot activeRoot frame gprs closeReadback returnReload
    (h : query entries returns incomingRoot closedRoot activeRoot frame gprs
      closeReadback returnReload 3 = 1) :
    accepted entries returns incomingRoot closedRoot activeRoot frame gprs
      closeReadback returnReload = true := by
  simpa [query] using h

example : query 2 1 0x1a0000 0x1a2000 0x1a2000 1 15 1 1 1 = 1 := by decide
example : query 2 0 0x1a0000 0x1a2000 0x1a2000 1 15 1 1 1 = 2 := by decide
example : query 2 1 0x1a0000 0x1a2000 0x1a2000 1 15 1 1 6 = 0 := by decide

@[export leanos_qotom_entry_integration_query]
def exportedQuery (entries returns incomingRoot closedRoot activeRoot frame gprs
    closeReadback returnReload word : UInt64) : UInt64 :=
  query entries returns incomingRoot closedRoot activeRoot frame gprs
