...
This is the canonical, version-one corpus for the two currently exported
...
theorem corpus_shape : vectors.length = 8 := by decide
...
theorem every_rejection_agrees :
    ...
    (vectors[6]).expected = 0 ∧ (vectors[7]).expected = 0 := by native_decide
