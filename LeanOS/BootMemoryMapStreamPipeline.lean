...
/-- Ignored payload and padding cannot alter the map, entry-count,
classification, or tag-count fields.  These equalities let the whole-tag
induction carry its exact tag budget across an already-packaged span. -/
...
theorem successfulIgnoredTagSpan_preserves_tag_fields
  ...
  | step state terminal chunk rest hphase haccepted hrest ih =>
      have hstep :
        ...
        rw [scalarStep_word state chunk (⟨2, by decide⟩ : Fin 19)] at haccepted
        simp only [BootMemoryMapStreamAuthority.stepWord] at haccepted
        rw [scalarStep_word state chunk (⟨10, by decide⟩ : Fin 19),
          scalarStep_word state chunk (⟨11, by decide⟩ : Fin 19),
          scalarStep_word state chunk (⟨14, by decide⟩ : Fin 19),
          scalarStep_word state chunk (⟨15, by decide⟩ : Fin 19),
          scalarStep_word state chunk (⟨18, by decide⟩ : Fin 19)]
        simp only [BootMemoryMapStreamAuthority.stepWord, hphase]
        split <;> simp_all
...
theorem canonicalIgnoredTag_successor
  ...
  have hroundedNat :
    ...
    have hmask (value : UInt64) :
        ((value + 7) &&& 0xfffffffffffffff8) = value / 8 * 8 := by
      bv_decide
    rw [hmask]
    apply UInt64.toNat.inj
